 ...
 ***************************************************************************/
peripheral_bus_driver g_proto_i2c = {
    // BUS_I2C,
    PERIPHERAL_I2C,
    {NULL},
    proto_i2c_init,
...
se_error_t api_connect(uint8_t *out_buf, uint32_t *out_buf_len)
{
    ...
    if (ret != SE_SUCCESS)
    {
        LOGE("failed to call acl_connect!\n");
        return ret;
    };
...
Function:		add_periph
Description:	增加设备句柄
            1. 增加设备句柄到g_drivers中，并返回设备句柄
Input:		driver 外设总线驱动句柄
                periph 外设句柄
Output: 		no
Return: 		外设句柄
Others: 		no
...
/*********************************************************************************
Function:		add_driver
Description:	增加外设总线驱动句柄和设备句柄
              1. 增加总线驱动句柄到g_drivers中，并返回驱动句柄
              2. 调用add_periph 函数增加设备句柄
Input:		driver 外设总线驱动句柄
                periph 外设句柄
Output: 		no
Return: 		外设总线驱动句柄
...
Description:	根据外设类型，从g_driver中查找外设总线驱动句柄
Input:		type 外设类型
Output: 		no
Return: 		外设总线驱动句柄
Others: 		no
...
Description:	根据外设总线驱动句柄和外设ID值，查找外设句柄
Input:		driver 外设总线驱动句柄
                        dev_id 外设ID
Output: 		no
Return: 		外设句柄
...
Description:	根据外设总线驱动类型和外设ID 值进行注册，即外设实例化
            1. 调用add_driver 函数增加总线驱动句柄和设备句柄
Input:		driver 外设总线驱动句柄
            periph 外设句柄
Output: 		no
Return: 		函数操作状态码
...
/*********************************************************************************
Function:		acl_select
Description:	从已注册的多个外设中，选择需要操作的外设
            1. 调用add_driver 函数增加总线驱动句柄和设备句柄
            2. 将查询到的总线驱动句柄和设备句柄存储到g_selected_driver
Input:		type  外设类型
            dev_id 外设ID值
Output: 		no
Return: 		函数操作状态码
...
Description:	断开与外设的连接
            1. 检查参数，加载选择外设的驱动，若只注册了一个外设，
            且未选择此外设，加载第一个注册的设备
            2. 根据加载的外设驱动，调用deinit 终止化函数和close函数
Input:		no
Output: 	no
Return:     函数操作状态码
Others: 	no
...
/*********************************************************************************
Function:		acl_reset
Description:	复位外设及获取外设ATR
            1. 检查参数，加载选择外设的驱动，若只注册了一个外设，
            且未选择此外设，加载第一个注册的设备
            2. 根据加载的外设驱动，调用reset复位函数
Input:		no
Output: 	out_buf 输出的起始地址
            out_buf_len  输出数据长度的起始地址
Return: 	函数操作状态码
...
Description:	发送控制命令
            1. 检查参数，加载选择外设的驱动，若只注册了一个外设，
            且未选择此外设，加载第一个注册的设备
            2. 根据加载的外设驱动，调用control控制函数
Input:		ctrlcode 命令控制码
            in_buf 输入的起始地址
            in_buf_len  输入数据长度
Output: 	out_buf 输出的起始地址
            out_buf_len  输出数据长度的起始地址
...
/*********************************************************************************
Function:       gpio_init
Description:  定义LED、调试 IO 等功能的GPIO 定义
Input:           no
Output:         no
Return:         no
Others:         no
...
Description:   SPI通信协议之复位设备的操作
                 1. 通过port层设备注册的函数列表指针，调用port层的lock函数上锁
                 2. 通过port层设备注册的函数列表指针，调用port层i2c接口的函数对设备复位
                 3. 调用proto_i2c_get_atr函数，获取ATR。
                 4. 通过port层设备注册的函数列表指针，调用unlock函数解锁。
Input:        periph 设备句柄
Output:      rbuf  待接收ATR的起始地址
             rlen  待接收ATR的长度
Return:      函数操作状态码
