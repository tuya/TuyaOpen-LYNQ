...
#define CSPI_TRANSFER_TRUNK_SIZE \
    7680  ///< Each DMA descriptor data size, fixed to 7680byte

#define ARM_CSPI_EVENT_TRANSFER_COMPLETE \
    (1UL << 0)  ///< Data Transfer completed
#define ARM_CSPI_EVENT_DATA_LOST \
    (1UL << 1)  ///< Data lost: Receive overflow / Transmit underflow
#define ARM_CSPI_EVENT_MODE_FAULT \
    (1UL << 2)  ///< Master Mode Fault (SS deactivated when Master)
...
typedef struct
{
    ...
    uint32_t
        outSwCtrl : 2;  ///< Data give for Rxfifo and whether need to binary
...
typedef struct
{
    ...
    uint32_t rsvd : 5;
    uint32_t rowScaleRatioLspi : 4;  ///< Every "rowScaleRatioLspi" lines
                                     ///< receive next line. 0: not scale
    uint32_t
        colScaleRatioLspi : 4;  ///< Every "colScaleRatioLspi" pixels receive
                                ///< next pixel in a line. 0: not scale
} cspiFrameProcLspi_t;
...
#define EC_CSPI_INDEX_START (0)
#define EC_CSPI_INDEX_LIMIT (2)

typedef enum CspiIdx_
{
    CSPI_INDEX_0 = 0,
    CSPI_INDEX_1,
} CspiIdx_e;

typedef enum CspiIoCtrl_
{
    OPEN_CSPI_IOCTL_POWER_ON = 0,       // 给CSPI上电，无参数
    OPEN_CSPI_IOCTL_POWER_OFF,          // 给CSPI下电，无参数
    OPEN_CSPI_IOCTL_SET_BUS_SPEED,      // 设置CSPI总线速度，参数为uint8_t*型，具体配置参考camFrequence_e
    OPEN_CSPI_IOCTL_SET_DATA_FORMAT,    // 设置CSPI数据格式，参数为cspiDataFmt_t*型
    OPEN_CSPI_IOCTL_FLUSH_RX_FIFO,      // flush接收DMA的FIFO，无参数
    OPEN_CSPI_IOCTL_SET_INTTERUPT,      // 设置CSPI中断使能，参数为cspiIntCtrl_t*型
    OPEN_CSPI_IOCTL_SET_RECV_TIMEOUT,   // 设置CSPI接收超时时间，参数为cspiTimeOutCycle_t*型
    OPEN_CSPI_IOCTL_SET_DMA_CTRL,       // 设置CSPI接收DMA配置，参数为cspiDmaCtrl_t*型
    OPEN_CSPI_IOCTL_SET_CSPI_CFG,       // 设置CSPI配置，参数为cspiCtrl_t*型
    OPEN_CSPI_IOCTL_SET_DELAY_CTRL,     // 设置CSPI延时控制，参数为cspiDelayCtrl_t*型
    OPEN_CSPI_IOCTL_START,              // 开始CSPI传输，无参数
    OPEN_CSPI_IOCTL_STOP,               // 停止CSPI传输，无参数
    OPEN_CSPI_IOCTL_SET_FRAME_INFO0,    // 设置CSPI Frame Info0，参数为cspiFrameInfo0_t*型
    OPEN_CSPI_IOCTL_SET_BINARY,         // 设置CSPI Binary控制，参数为cspiBinaryCtrl_t*型
    OPEN_CSPI_IOCTL_AUTO_CG_CTRL,       // 设置CSPI自动配置控制，参数为cspiAutoCgCtrl_t*型
    OPEN_CSPI_IOCTL_SET_RESOLUTION,     // 设置CSPI分辨率，参数为uint8_t*型，具体配置参考camResolution_e
    OPEN_CSPI_IOCTL_SET_FRAME_PROC,     // 设置CSPI帧处理配置，参数为cspiFrameProcLspi_t*型
    OPEN_CSPI_IOCTL_TRANS_ABOUT,        // 中断传输，无参数
    OPEN_CSPI_IOCTL_CHECK_FRAME_VALID,  // 查询CSPI是否帧接收完成
    OPEN_CSPI_IOCTL_CLEAR_FRAME_VALID,  // 清除CSPI帧接收完成标志位
    OPEN_CSPI_IOCTL_GET_ERR_STATUS,     // 获取CSPI错误状态，参数为CamSpiErr_e*型
    OPEN_CSPI_IOCTL_START_RECV,         // 开始传输，参数为接收地址，uint8_t*型
    OPEN_CSPI_IOCTL_SET_CBCTRL,         // 配置CBCTRL状态，收到帧时调用无参数。
} CspiIoCtrl_e;

typedef struct CspiCfg_
{
    cspiPins_t pins;
    cspiCbEvent_fn cbEvent;
} CspiCfg_t;

int api_cspi_startup(void *para);
uint32_t api_cspi_create(CspiIdx_e index, CspiCfg_t *config);
