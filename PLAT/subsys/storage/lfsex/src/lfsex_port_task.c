...
#define LFS_BLOCK_DEVICE_TOTOAL_SIZE    (EF_IMG_LFS_SIZE)
#define LFS_BLOCK_DEVICE_LOOK_AHEAD     (256)
...
PLAT_FPSRAM_ZI_CUST static uint8_t                  gLfsDaemonTaskStack[LFS_TASK_STACK_SIZE];
#else
...
#ifdef LFSEX_WITHOUT_THREAD
static osMutexId_t gLfsexMutex = NULL;
#endif
        ...
        lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    spiFlashRead((EF_IMG_LFS_LNA + block * cfg->block_size + off), (uint8_t *)buffer, size);
        ...
        lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    spiFlashWritePage((EF_IMG_LFS_LNA + block * cfg->block_size + off), (uint8_t *)buffer, size);
...
static int block_device_erase(const struct lfs_config *cfg, lfs_block_t block)
{
    ...
    spiFlashEraseSector(EF_IMG_LFS_LNA + block * cfg->block_size);
...
void LFSEX_loadMonitorResult(void)
{
    int32_t err;
...
void LFSEX_saveMonitorResult(void)
{
...
static void LFS_createDaemonTaskAndQueue(void)
{
    ...
    taskAttr.priority = osPriorityNormal1;   // lower priority after system initialization phase
    ...
    Service_reg(serviceName, LFS_daemonTaskEntry, NULL, taskAttr.cb_mem, taskAttr.cb_size, taskAttr.stack_mem, taskAttr.stack_size, taskAttr.priority);
    gLfsDaemonTaskthreadId = (osThreadId_t)Service_start(serviceName);
#endif
    ...
    while(1)
    {
        ...
        if(status == osOK)
        {
            switch(request.type)
            {
                ...
                // dir operations
                case LFS_REQUEST_MK_DIR:

                    ret = lfs_mkdir(&lfs, (char *)request.data[0]);
                    LFS_reply(request.threadId, ret);
...
int LFSEX_init(void)
{
    ...
    if (gLfsexMutex == NULL)
    {
        gLfsexMutex = osMutexNew(NULL);
        if (gLfsexMutex == NULL)
        {
            ECPLAT_PRINTF(UNILOG_LFS, LFSEX_init, P_ERROR, "Failed to create mutex for gLfsexMutex.");
        }
    }
#else
...
#ifdef LFSEX_WITHOUT_THREAD
    osMutexAcquire(gLfsexMutex, osWaitForever);
    ret = lfs_format(&lfs, &lfs_cfg);
    osMutexRelease(gLfsexMutex);
#else
...
lfs_ssize_t LFSEX_fileRead(lfs_file_t *file, void *buffer, lfs_size_t size)
{
    ...
    ECPLAT_PRINTF(UNILOG_PLA_DRIVER, LFSEX_fileRead1, P_DEBUG, "LFS_TEST LFSEX_fileRead1");
#ifdef LFSEX_WITHOUT_THREAD
    osMutexAcquire(gLfsexMutex, osWaitForever);
...
#endif
    ECPLAT_PRINTF(UNILOG_PLA_DRIVER, LFSEX_fileRead2, P_DEBUG, "LFS_TEST LFSEX_fileRead2");
...
int LFSEX_mkdir(const char *path)
{
    int ret;
    ...
    osMutexRelease(gLfsexMutex);
#else
    ret = LFS_waitForCompletion(LFS_REQUEST_MK_DIR, NULL, (uint32_t)path, 0);
#endif
