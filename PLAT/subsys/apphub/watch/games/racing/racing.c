...
#define EPAT_LOG(subId, debugLevel, format, ...)  \
    ECPLAT_PRINTF(UNILOG_OPEN_APP, subId, debugLevel, format, ##__VA_ARGS__)
...
int highScore, currentScore, currentSpeed;
...
/**
  ...
  \return
*/
static lv_event_code_t event_update_label_distance;
static lv_event_code_t event_update_label_speed;
static void user_event_callback(lv_event_t *e) 
{
    lv_obj_t *label = lv_event_get_target(e);
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == event_update_label_distance) {
        ...
        // SYSLOG_INFO("event_code=0x%x,label=0x%x;%d,0x%x;%d,0x%x\r\n",event_code,label,data[0],data,currentScore,&currentScore);
    } else if (event_code == event_update_label_speed) {
        int *pdata = (int *)lv_obj_get_user_data(e->target);
        // int *pdata = (int *)lv_event_get_param(e);
        // lv_label_set_text(label, data);
        lv_label_set_text_fmt(label, "%d", *pdata);
        // SYSLOG_INFO("event_code=0x%x,label=0x%x;%d,0x%x;%d,0x%x\r\n",event_code,label,*pdata,pdata,currentSpeed,&currentSpeed);
    }
}
/**
  \fn
...
void ui_raceScreen_screen_init(void (*callback)(const char *, const lv_img_dsc_t *, lv_obj_t **))
{
    ...
    active = false;
    task_list_add(ui_raceScreen, ui_raceScreen_screen_loop, "race is active!",10);
    event_update_label_distance = lv_event_register_id();
    event_update_label_speed = lv_event_register_id();
    lv_obj_add_event_cb(ui_distanceLabel, user_event_callback, event_update_label_distance, NULL);
    lv_obj_add_event_cb(ui_speedLabel, user_event_callback, event_update_label_speed, NULL);
    SYSLOG_INFO("event_code=0x%x,label=0x%x\r\n",event_update_label_distance, ui_distanceLabel);
    SYSLOG_INFO("event_code=0x%x,label=0x%x\r\n",event_update_label_speed, ui_speedLabel);
#endif
}
/**
  \fn
...
void ui_raceScreen_screen_loop(void *arg)
{
    ...
    // EPAT_LOG(ui_race_screen_loop, P_INFO, "%s",message);
    // LV_LOG_USER("Callback executed: %s", message);
#ifdef ENABLE_GAME_RACING
    if (!active)
    {
        return;
    ...
    if (isRacing)
    {
        ...
        lv_event_send(ui_distanceLabel, event_update_label_distance, &currentScore);
        ...
        currentSpeed = (int)(speedFactor * 100);
        lv_obj_set_user_data(ui_speedLabel, &currentSpeed);
        lv_event_send(ui_speedLabel, event_update_label_speed, NULL);
