...
void _lv_log_add(lv_log_level_t level, const char * file, int line, const char * func, const char * format, ...)
{
    ...
    if(level >= LV_LOG_LEVEL) {
        ...
        static const char * lvl_prefix[] = {"\033[37mTrace", "\033[32mInfo", "\033[33mWarn", "\033[31mError", "\033[34mUser"};

#if LV_LOG_PRINTF
        printf("\r[%s]\t(%04" LV_PRId32 ", +%" LV_PRId32 ")\t %s: ",
               lvl_prefix[level], t, t - last_log_time, func);
        vprintf(format, args);
        printf(" \t(in %s line #%d)\r\n", &file[p], line);
#else
        if(custom_print_cb) {
            ...
            lv_snprintf(buf, sizeof(buf), "\r[%s]\t(%04" LV_PRId32 ", +%" LV_PRId32 ")\t %s: %s \t(in %s line #%d)\r\n",
                        lvl_prefix[level], t, t - last_log_time, func, msg, &file[p], line);
#else
            lv_vaformat_t vaf = {format, &args};
            lv_snprintf(buf, sizeof(buf), "\r[%s]\t(%04" LV_PRId32 ", +%" LV_PRId32 ")\t %s: %pV \t(in %s line #%d)\r\n",
                        lvl_prefix[level], t, t - last_log_time, func, (void *)&vaf, &file[p], line);
#endif
            custom_print_cb(buf);
        }
#endif
