...
if(LV_COLOR_DEPTH==16 && LV_IMG_PX_SIZE_ALPHA_BYTE==3 && cf==LV_IMG_CF_TRUE_COLOR_ALPHA && draw_dsc->recolor_opa<=LV_OPA_MIN && !mask_any && !transform)
{
    //blend_dsc.blend_area = &blend_area;
    //blend_dsc.src_buf = (lv_color_t *)src_buf;
    //blend_dsc.mask_buf = src_buf + 2;
    //blend_dsc.mask_res = LV_DRAW_MASK_RES_CHANGED;
    ...
    int32_t x,y,u0,u1,u2;
    lv_coord_t stride;
    uint8_t *psrc,*pbuf,*pbuf2;
    lv_color_t * pdst;

    stride = lv_area_get_width(draw_ctx->buf_area);
    psrc = (uint8_t *)(src_buf + ((blend_area.y1-coords->y1)*src_h + (blend_area.x1-coords->x1))*3), 

    pdst = draw_ctx->buf;
    pdst += stride * (blend_area.y1 - draw_ctx->buf_area->y1) + (blend_area.x1 - draw_ctx->buf_area->x1);
//lv_log("lv_draw_sw_img_decoded: %d_%d_%d_%d",blend_area.x1,blend_area.x2,blend_area.y1,blend_area.y2);
//lv_log("lv_draw_sw_img_decoded: %d_%d_%d_%d",coords->x1,coords->x2,coords->y1,coords->y2);

    if(draw_ctx->wait_for_finish) draw_ctx->wait_for_finish(draw_ctx);

    pbuf = lv_mem_buf_get(blend_w*3);

    for(y = 0; y < blend_h; y++)
    {
        if(pbuf != NULL)
        {
            memcpy(pbuf,psrc,blend_w*3);
            pbuf2 = pbuf;
        }
        else
        {
        ...
        for(x = 0; x < blend_w; x++) 
        {
            ...
            if(u0 >= (((LV_OPA_COVER>>3)<<3)-4))
            {
                pdst->full = pbuf2[1]<<8 | pbuf2[0];
            }
            else if(u0 >= 4)
            {
                u1 = pbuf2[1]<<8 | pbuf2[0];

                u0 = (u0 + 4) >> 3;
                u1 = (u1 | u1 << 16) & 0x7E0F81F;
                u2 = (uint32_t)((uint32_t)pdst->full | ((uint32_t)pdst->full << 16)) & 0x7E0F81F;
                u0 = ((((u1 - u2) * u0) >> 5) + u2) & 0x7E0F81F;
                pdst->full = (uint16_t)((u0 >> 16) | u0);                    
            }
