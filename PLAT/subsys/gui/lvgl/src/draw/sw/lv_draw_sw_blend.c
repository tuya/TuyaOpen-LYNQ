...
#ifdef FEATURE_MES_LCD_ENABLE
    uint32_t cost_us = measure_time(&cost_tick);    //[-68,206][12][135,45],594
    if(cost_us > cost_max){
        cost_max = cost_us;
        SYSLOG_INFO("%d\r\n", cost_us);
    }
...
//        uint32_t c32 = color.full + ((uint32_t)color.full << 16);
        uint32_t c32 = (uint32_t)((uint32_t)color.full | ((uint32_t)color.full << 16)) & 0x7E0F81F;
        uint32_t u0,u1;
#endif
        ...
        if(opa >= LV_OPA_MAX) {
            ...
            for(y = 0; y < h; y++) {
                ...
                for(x = 0; x < w; x++) 
                {
                    ...
                    {
                        u0 = (u0 + 4) >> 3;
                        u1 = (uint32_t)((uint32_t)dest_buf->full | ((uint32_t)dest_buf->full << 16)) & 0x7E0F81F;
                        u0 = ((((c32 - u1) * u0) >> 5) + u1) & 0x7E0F81F;
                        dest_buf->full = (uint16_t)((u0 >> 16) | u0);
                    }
        ...
        /*With opacity*/
            ...
            for(y = 0; y < h; y++) {
                ...
                for(x = 0; x < w; x++) 
                {
                    ...
                    {
                        u0 = (u0 + 4) >> 3;
                        u1 = (uint32_t)((uint32_t)dest_buf[x].full | ((uint32_t)dest_buf[x].full << 16)) & 0x7E0F81F;
                        u0 = ((((c32 - u1) * u0) >> 5) + u1) & 0x7E0F81F;
                        dest_buf[x].full = (uint16_t)((u0 >> 16) | u0);
                    }
                }
#else
                for(x = 0; x < w; x++) {
