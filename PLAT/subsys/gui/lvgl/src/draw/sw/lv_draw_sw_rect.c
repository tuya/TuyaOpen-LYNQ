...
void lv_draw_sw_rect(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    #ifdef FEATURE_MES_LCD_ENABLE
    uint32_t cost_us = measure_time(&cost_tick);    //[0,0],[239,319],3736
    if(cost_us > cost_max1){
        cost_max1 = cost_us;
        SYSLOG_INFO("[%d,%d],[%d,%d],%d\r\n",coords->x1,coords->y1,coords->x2,coords->y2,cost_us);
    }
    #endif
}
...
void lv_draw_sw_bg(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    draw_bg_img(draw_ctx, dsc, coords);
    #ifdef FEATURE_MES_LCD_ENABLE
    uint32_t cost_us = measure_time(&cost_tick);    //[45,80][12][135,45],1918us
    if(cost_us > cost_max2){
        cost_max2 = cost_us;
        SYSLOG_INFO("[%d,%d],[%d,%d],%d\r\n",coords->x1,coords->y1,coords->x2,coords->y2,cost_us);
    }
    #endif
