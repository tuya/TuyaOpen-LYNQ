...
static uint32_t  __attribute__((used)) _volc_remove_file_dir(uint64_t data, volc_dir_entry_type_e type, char* path, char* name) {
    VOLC_UNUSED_PARAM(data);
    VOLC_UNUSED_PARAM(name);
	VOLC_UNUSED_PARAM(type);
	VOLC_UNUSED_PARAM(path);
    uint32_t ret = VOLC_STATUS_REMOVE_DIRECTORY_FAILED;
#if 0
    switch (type) {
...
static uint32_t __attribute__((used)) _volc_get_file_dir_size(uint64_t data, volc_dir_entry_type_e type, char* path, char* name) {
    VOLC_UNUSED_PARAM(data);
    VOLC_UNUSED_PARAM(name);
	VOLC_UNUSED_PARAM(type);
	VOLC_UNUSED_PARAM(path);
	uint32_t ret = VOLC_STATUS_UNKNOWN_DIR_ENTRY_TYPE;
#if 0
    uint64_t size = 0;
...
uint32_t volc_traverse_directory(const char* path, uint64_t data, bool iterate, volc_directory_entry_callback entry) {
	VOLC_UNUSED_PARAM(path);
	VOLC_UNUSED_PARAM(data);
	VOLC_UNUSED_PARAM(iterate);
	VOLC_UNUSED_PARAM(entry);
    uint32_t ret = VOLC_STATUS_DIRECTORY_OPEN_FAILED;
#if 0
...
uint32_t volc_create_directory(const char* path) {
	VOLC_UNUSED_PARAM(path);
	return VOLC_STATUS_DIRECTORY_ENTRY_STAT_ERROR;
#if 0
    struct stat st;
    if (stat(path, &st) == 0)
    {
...
uint32_t volc_get_directory_size(const char* path, uint64_t* p_size) {
	VOLC_UNUSED_PARAM(path);
	VOLC_UNUSED_PARAM(p_size);
   uint32_t ret = VOLC_STATUS_DIRECTORY_ENTRY_STAT_ERROR;
#if 0
    uint64_t size = 0;
...
__attribute__((weak)) int mkdir(const char *_path, mode_t __mode)
{
 	return VOLC_STATUS_DIRECTORY_ENTRY_STAT_ERROR;
}