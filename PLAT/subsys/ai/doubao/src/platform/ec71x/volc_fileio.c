...
uint32_t volc_file_read(const char* path, bool bin_mode, uint8_t* buffer, uint64_t* size) {
    ...
    VOLC_CHK((file_fread(buffer, (size_t) file_len, 1, fp) == 1), VOLC_STATUS_READ_FILE_FAILED);
...
uint32_t volc_file_read_segment(const char* path, bool bin_mode, uint8_t* buffer, uint64_t offset, uint64_t size) {
    ...
    VOLC_CHK(result == 0 && (file_fread(buffer, (size_t) size, 1, fp) == 1), VOLC_STATUS_READ_FILE_FAILED);
...
uint32_t volc_file_write(const char* path, bool bin_mode, bool append, uint8_t* buffer, uint64_t size) {
    ...
    fp = file_fopen(path, bin_mode ? (append ? "ab" : "wb") : (append ? "a" : "w"));

    VOLC_CHK(fp != NULL, VOLC_STATUS_OPEN_FILE_FAILED);

    // Write the buffer to the file
    VOLC_CHK(file_fwrite(buffer, (size_t) size, 1, fp) == 1, VOLC_STATUS_WRITE_TO_FILE_FAILED);
...
uint32_t volc_file_update(const char* path, bool bin_mode, uint8_t* buffer, uint64_t offset, uint64_t size) {
    ...
    VOLC_CHK(0 == file_fseek(fp, (uint32_t) offset, SEEK_SET), VOLC_STATUS_INVALID_OPERATION);
...
uint32_t volc_file_set_length(const char* path, uint64_t length) {
    ...
    ret_val = file_truncate(path, length);
...
uint32_t volc_file_exists(const char* path, bool* p_exists) {
    ...
    struct stat st;
    int32_t result = file_fstat(path, &st);
    *p_exists = (result == 0);
...
uint32_t volc_file_create(const char* path, uint64_t size) {
    ...
    fp = file_fopen(path, "w+b");
    VOLC_CHK(fp != NULL, VOLC_STATUS_OPEN_FILE_FAILED);

    if (size != 0) {
        VOLC_CHK(0 == file_fseek(fp, (uint32_t)(size - 1), SEEK_SET), VOLC_STATUS_INVALID_OPERATION);
...
__attribute__((weak)) void _unlink()
{
}
