...
uint32_t volc_encrypt_or_decrypt(bool encrypt, const char* key, uint64_t iv_number, const unsigned char* input, uint32_t ilen, unsigned char* output, uint32_t* olen) {
    ...
    mbedtls_cipher_setkey(&aes_ctr_128_ctr, (const unsigned char*)key, VOLC_AES128_KEY_LENGTH * 8, mode);
...
uint32_t volc_pk_verify(const char* pub_key, size_t pub_key_len, const char* credential, size_t credential_len, const char* signature, size_t signature_len) {
    ...
    mbedtls_pk_init(&pk);
    mbedtls_pk_parse_public_key(&pk, (const unsigned char*)pub_key, pub_key_len);
    ...
    ret = mbedtls_md(md_info, (const unsigned char* )credential, credential_len, hash);
    if (0 != ret) {
        ret = VOLC_STATUS_INTERNAL_ERROR;
        goto err_out_label;
    }

    ret = mbedtls_pk_verify(&pk, mbedtls_md_get_type(md_info), hash, mbedtls_md_get_size(md_info), (const unsigned char* )signature, signature_len);
    if (ret != 0) {
