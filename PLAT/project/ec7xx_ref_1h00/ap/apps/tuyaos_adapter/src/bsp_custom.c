...
__USED static uint8_t rsvUsrRam[115*1024];
#else
__USED static uint8_t rsvUsrRam[120*1024];
#endif
#else
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[295*1024];
#else
...
#else
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[195*1024];
#else
...
#else
__USED static uint8_t rsvUsrRam[419*1024];     // 420KB ram reserved for customer
#endif
#endif
#endif//ims enable
#else//not open cpu
...
static void BSP_InitUartHandler(uint8_t uartIndex, uint32_t baudRate, uint32_t frameFormat, BSP_SetConfigFunc setConfigFn)
{
    ...
    EC_ASSERT(uartDevConf.drvHandler, uartDevConf.drvHandler, setConfigFn, 0);
...
{
#ifdef TYPE_EC718M
    *pBase = PSRAM_P2_START_ADDR|PSRAM_PCACHE2_BASE;
    return PSRAM_P2_LENGTH;
#else
...
void Pad2_WakeupIntHandler(void)
{
    ...
    if(tkl_cellular_get_sim_hotplug_status(0) == false) {
        ECPLAT_PRINTF_OPT(UNILOG_PMU, Pad2_WakeupIntHandler_3, P_VALUE, "Pad2 Wakeup");
        agpio_irq_callback(79);
    } else {
        NVIC_DisableIRQ(PadWakeup2_IRQn);
        uint8_t level = (slpManGetWakeupPinValue() >> PadWakeup2_IRQn) & 0x1;
        if(level == sim_insert_level) {
            //插入sim卡
            ECPLAT_PRINTF(UNILOG_PMU, Pad2_WakeupIntHandler_2, P_VALUE, "Pad2 simcard insert");
            simcard_plug_handle(true);
        } else {
            //拔出sim卡
            ECPLAT_PRINTF(UNILOG_PMU, Pad2_WakeupIntHandler_1, P_VALUE, "Pad2 simcard remove");
...
* Output:      none.
*===========================================================================*/
/* L511G middleware (atec_plat_dev) references these globals; L511C middleware does not. */
...
uint8_t mbtk_get_cam_bus_num(void)
{
    return cam_bus_num;
}
