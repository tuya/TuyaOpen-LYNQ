...
#define kBlockSizeAlign       (1 << 12)   // alignment for block allocation
...
PLAT_BL_UNCOMP_FLASH_TEXT static void LzInWindow_Free(CMatchFinder *p, ISzAllocPtr alloc)
{
  if (!p->directInput)
  {
...
PLAT_BL_UNCOMP_FLASH_TEXT static int LzInWindow_Create2(CMatchFinder *p, UInt32 blockSize, ISzAllocPtr alloc)
{
  if (blockSize == 0)
    return 0;
...
#else
    p->bufferBase = (Byte *)(lzmaBootBuffBase + LZMA_BOOT_BUFF_BLOCKSIZE_OFFSET);
#endif
    // printf("\nbufferBase = %p\n", p->bufferBase);
    // return 0; // for debug
  }
...
PLAT_BL_UNCOMP_FLASH_TEXT static const Byte *MatchFinder_GetPointerToCurrentPos(CMatchFinder *p) { return p->buffer; }

PLAT_BL_UNCOMP_FLASH_TEXT static UInt32 MatchFinder_GetNumAvailableBytes(CMatchFinder *p) { return GET_AVAIL_BYTES(p); }
...
PLAT_BL_UNCOMP_FLASH_TEXT int MatchFinder_NeedMove(CMatchFinder *p)
{
  if (p->directInput)
    return 0;
  if (p->streamEndWasReached || p->result != SZ_OK)
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_ReadIfRequired(CMatchFinder *p)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT static void MatchFinder_SetDefaultSettings(CMatchFinder *p)
{
  p->cutValue = 32;
  p->btMode = 1;
  p->numHashBytes = 3;
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Construct(CMatchFinder *p)
{
  unsigned i;
  p->bufferBase = NULL;
  p->directInput = 0;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void MatchFinder_FreeThisClassMemory(CMatchFinder *p, ISzAllocPtr alloc)
{
  ISzAlloc_Free(alloc, p->hash);
  p->hash = NULL;
}

PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Free(CMatchFinder *p, ISzAllocPtr alloc)
{
  MatchFinder_FreeThisClassMemory(p, alloc);
  LzInWindow_Free(p, alloc);
}

PLAT_BL_UNCOMP_FLASH_TEXT static CLzRef* AllocRefs(size_t num, ISzAllocPtr alloc)
{
...
#else
  return (CLzRef *)(lzmaBootBuffBase + LZMA_BOOT_BUFF_HASH_OFFSET);
#endif
}
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32 GetBlockSize(CMatchFinder *p, UInt32 historySize)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT int MatchFinder_Create(CMatchFinder *p, UInt32 historySize,
...
PLAT_BL_UNCOMP_FLASH_TEXT static void MatchFinder_SetLimits(CMatchFinder *p)
{
  UInt32 k;
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Init_LowHash(CMatchFinder *p)
{
  size_t i;
  CLzRef *items = p->hash;
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Init_HighHash(CMatchFinder *p)
{
  size_t i;
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Init_4(CMatchFinder *p)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_Init(CMatchFinder *p)
{
...
ATTRIB_SSE41
#endif
PLAT_BL_UNCOMP_FLASH_TEXT void
MY_FAST_CALL
...
LZMA_BSS_SECTION static LZFIND_SATUR_SUB_CODE_FUNC g_LzFind_SaturSub;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void SkipMatchesSpec(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son,
    size_t _cyclicBufferPos, UInt32 _cyclicBufferSize, UInt32 cutValue)
{
  CLzRef *ptr0 = son + ((size_t)_cyclicBufferPos << 1) + 1;
  CLzRef *ptr1 = son + ((size_t)_cyclicBufferPos << 1);
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32* Bt2_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT UInt32* Bt3Zip_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32* Bt3_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  UInt32 mmm;
  UInt32 h2, d2, pos;
  unsigned maxLen;
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32* Bt4_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  UInt32 mmm;
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32* Bt5_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  UInt32 mmm;
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32* Hc4_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  UInt32 mmm;
  UInt32 h2, h3, d2, d3, pos;
  unsigned maxLen;
...
PLAT_BL_UNCOMP_FLASH_TEXT static UInt32 * Hc5_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  UInt32 mmm;
  UInt32 h2, h3, d2, d3, maxLen, pos;
  UInt32 *hash;
...
PLAT_BL_UNCOMP_FLASH_TEXT UInt32* Hc3Zip_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  GET_MATCHES_HEADER(3)
  HASH_ZIP_CALC;
  curMatch = p->hash[hv];
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Bt2_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  SKIP_HEADER(2)
  {
    HASH2_CALC;
...
PLAT_BL_UNCOMP_FLASH_TEXT void Bt3Zip_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  SKIP_HEADER(3)
  {
    HASH_ZIP_CALC;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Bt3_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  SKIP_HEADER(3)
  {
    UInt32 h2;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Bt4_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  SKIP_HEADER(4)
  {
    UInt32 h2, h3;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Bt5_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  SKIP_HEADER(5)
  {
    UInt32 h2, h3;
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Hc4_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  HC_SKIP_HEADER(4)
...
PLAT_BL_UNCOMP_FLASH_TEXT static void Hc5_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  HC_SKIP_HEADER(5)
...
PLAT_BL_UNCOMP_FLASH_TEXT void Hc3Zip_MatchFinder_Skip(CMatchFinder *p, UInt32 num)
{
  HC_SKIP_HEADER(3)
...
PLAT_BL_UNCOMP_FLASH_TEXT void MatchFinder_CreateVTable(CMatchFinder *p, IMatchFinder2 *vTable)
{
...
PLAT_BL_UNCOMP_FLASH_TEXT void LzFindPrepare()
{
  #ifndef FORCE_SATUR_SUB_128
  #ifdef USE_SATUR_SUB_128
