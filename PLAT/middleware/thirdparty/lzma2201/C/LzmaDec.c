...
PLAT_BL_UNCOMP_FLASH_TEXT static int MY_FAST_CALL LzmaDec_DecodeReal2(CLzmaDec *p, SizeT limit, const Byte *bufLimit)
{
  if (p->checkDicSize == 0)
  {
...
PLAT_BL_UNCOMP_FLASH_TEXT static ELzmaDummy LzmaDec_TryDummy(const CLzmaDec *p, const Byte *buf, const Byte **bufOut)
{
  UInt32 range = p->range;
  UInt32 code = p->code;
...
PLAT_BL_UNCOMP_FLASH_TEXT void LzmaDec_Init(CLzmaDec *p)
{
  p->dicPos = 0;
...
PLAT_BL_UNCOMP_FLASH_TEXT SRes __attribute__((noinline)) LzmaDec_DecodeToBuf(CLzmaDec *p, Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen, ELzmaFinishMode finishMode, ELzmaStatus *status)
{
  volatile SizeT outSize = *destLen;
  volatile SizeT inSize = *srcLen;
  *srcLen = *destLen = 0;
  for (;;)
  {
    volatile SizeT inSizeCur = inSize, outSizeCur, dicPos;
    volatile ELzmaFinishMode curFinishMode;
    volatile SRes res;
    ...
    res = LzmaDec_DecodeToDic(p, outSizeCur, src, (SizeT *)(&inSizeCur), curFinishMode, status);
    src += inSizeCur;
    inSize -= inSizeCur;
    *srcLen += inSizeCur;
    outSizeCur = p->dicPos - dicPos;
    char *destTempLocal = (char *)(dest);
    char *dicTempLocal = (char *)(p->dic + dicPos);
    int i = 0;
    for(i=0; i<outSizeCur; i++)
    {
        destTempLocal[i] = dicTempLocal[i];
...
PLAT_BL_UNCOMP_FLASH_TEXT static void LzmaDec_FreeDict(CLzmaDec *p, ISzAllocPtr alloc)
{
  ISzAlloc_Free(alloc, p->dic);
  p->dic = NULL;
}

PLAT_BL_UNCOMP_FLASH_TEXT void LzmaDec_Free(CLzmaDec *p, ISzAllocPtr alloc)
{
  LzmaDec_FreeProbs(p, alloc);
  LzmaDec_FreeDict(p, alloc);
}

PLAT_BL_UNCOMP_FLASH_TEXT SRes LzmaProps_Decode(CLzmaProps *p, const Byte *data, unsigned size)
{
  UInt32 dicSize;
  Byte d;
...
PLAT_BL_UNCOMP_FLASH_TEXT static SRes LzmaDec_AllocateProbs2(CLzmaDec *p, const CLzmaProps *propNew, ISzAllocPtr alloc)
{
  ...
  if (!p->probs || numProbs != p->numProbs)
  {
    ...
    #ifdef CORE_IS_AP
    extern UINT32 decompBuffStartAddr;
    p->probs = (CLzmaProb *)((int)decompBuffStartAddr + (int)AP_DECOMP_MEM_PROBS_OFFSET);        
    #endif
	
    #ifdef CORE_IS_CP
    extern UINT32 decompBuffStartAddr;
    p->probs = (CLzmaProb *)((int)decompBuffStartAddr + (int)CP_DECOMP_MEM_PROBS_OFFSET);
    #endif
...
PLAT_BL_UNCOMP_FLASH_TEXT SRes LzmaDec_AllocateProbs(CLzmaDec *p, const Byte *props, unsigned propsSize, ISzAllocPtr alloc)
{
  CLzmaProps propNew;
...
//char lzmaDisBuf[65536]={0};

PLAT_BL_UNCOMP_FLASH_TEXT SRes LzmaDec_Allocate(CLzmaDec *p, const Byte *props, unsigned propsSize, ISzAllocPtr alloc)
{
  CLzmaProps propNew;
  SizeT dicBufSize;
  RINOK(LzmaProps_Decode(&propNew, props, propsSize));
  ...
  {
    LzmaDec_FreeDict(p, alloc);
    //p->dic = lzmaDisBuf;//(Byte *)ISzAlloc_Alloc(alloc, dicBufSize);
    
    #ifdef CORE_IS_AP
    extern UINT32 decompBuffStartAddr;
    p->dic  = (Byte *)((int)decompBuffStartAddr + (int)AP_DECOMP_MEM_DIC_OFFSET);
    #endif  
    
    #ifdef CORE_IS_CP
    extern UINT32 decompBuffStartAddr;
    p->dic  = (Byte *)((int)decompBuffStartAddr + (int)CP_DECOMP_MEM_DIC_OFFSET);    
    #endif  
...
PLAT_BL_UNCOMP_FLASH_TEXT SRes LzmaDecode(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
