...
#ifdef FEATURE_PS_REF_CR_TCPIP_SOCK_AT_ENABLE
#define AT_SOC_REF_CR_DL_LENGTH_MAX MIPRD_1_READ_LEN_MAX
#endif
...
#ifdef FEATURE_PS_REF_CR_TCPIP_SOCK_AT_ENABLE
#define AT_REFCR_SOCK_SUPPORT_CONNECT_MAX 6
#endif
...
/*
 * APPL SOCK TYPE ID: used for timer expire
*/
typedef enum applSockTypeId_Enum
{
    APPL_SOCKET_ECSOCK_TYPE = 0,
    APPL_SOCKET_QISOCK_TYPE = 1,
    APPL_SOCKET_CMREFSOCK_TYPE = 2,

    APPL_SOCKET_TYPE_MAX = 7
}ApplSockTypeId;
...
/***********AT CM REF MIP SOC **************************/
    ATECMIPREQ_CREATE   = 70,
    ATECMIPREQ_DELETE,
    ATECMIPREQ_SETDATAACCMODE,
    ATECMIPREQ_GETDATAACCMODE,
    ATECMIPREQ_SEND,
    ATECMIPREQ_SEND_PSTH_DATA,
    ATECMIPREQ_SET_CFG,
    ATECMIPREQ_GET_STATE,
    ATECMIPREQ_GET_SACK,
    ATECMIPREQ_GET_BUF_INFO,
    ATECMIPREQ_GET_BUF_DATA,
...
typedef enum applSockPrimId_Enum
{
    ...
    APPL_REFCR_MIPSOCKET_CREATE_IND,
    APPL_REFCR_MIPREAD_BUFF_DATA_CNF,
    APPL_REFCR_MIPSOCKET_SETMODE_CNF,
    APPL_REFCR_MIPSOCKET_GETMODE_CNF,
    APPL_REFCR_MIPSOCKET_SEND_CNF,
    APPL_REFCR_MIPSOCKET_SENDPSTH_CNF,
    APPL_REFCR_MIPSOCKET_CLOSE_IND,

    APPL_REFCR_MIPSOCKET_CONN_ERR_IND,
    APPL_REFCR_MIPSOCKET_TCP_ACK_IND,
    APPL_REFCR_MIPSOCKET_TCP_DATA_IND,
    APPL_REFCR_MIPSOCKET_TCP_BUFF_IND,
    APPL_REFCR_MIPSOCKET_UDP_DATA_IND,
    APPL_REFCR_MIPSOCKET_UDP_BUFF_IND,
...
typedef struct AtecSktPriMgrContext_Tag{
    UINT8  connectId;      /*connectId 0~11*/
    UINT8  rsvd0;
    UINT16 rsvd1;
...
{
    struct AtRefSocketSendInfo_Tag *pNext;

    UINT8   connectId;
    UINT8   source;
    UINT16  reqHander;  /*channel ID is the key for different sending data info*/
    ...
    UINT8   sendingDataWaitTime; //1~180s
    UINT8   rsvd0;
    UINT16  rsvd1;

    ip_addr_t remoteAddr;
    osTimerId_t sendTimeoutTimer;
    ...
    void    *pPdu;  /*Dyn mem, raw data send to SOCK task, for feature, could be UlPduBlock
                    ...
                    * It will be free in cms sock mgr task after send success.
                    */
}AtRefSocketSendInfo;

#ifdef FEATURE_PS_REF_CR_TCPIP_SOCK_AT_ENABLE

typedef enum AtRefCrSockStatus_Tag{
    REF_CR_SOCK_INIT = 0,
    REF_CR_SOCK_CONNECTING = 1,
    REF_CR_SOCK_CONNECTED = 2,
    REF_CR_SOCK_CLOSING = 3,
    REF_CR_SOCK_CLOSED = 4,
}AtRefCrSockStatus;

typedef enum AtRefCrDataAccessMode_tag {
    ATEC_REF_CR_DATA_ACC_BUFF_MODE,
    ATEC_REF_CR_DATA_ACC_DIRECT_MODE,
    ATEC_REF_CR_DATA_ACC_PASSTHROUGH_MODE,
}AtRefCrDataAccessMode;


typedef struct AtRefCrSocketCreateReq_Tag{
    UINT8  serviceType; //AtRefSocServiceType
    UINT8  connectId;   //0~5
    UINT8  accessMode;  //0~3
    UINT8  timeout;     //1~180
    UINT16 remotePort;  //0~65535
    UINT16 localPort;   //0~65535
    UINT32 reqSource;
    void  *eventCallback;
    CHAR   remote[AT_SOC_MAX_REF_URL_IPADDR_LEN+1];//remote addr or url
}AtRefCrSocketCreateReq;

typedef struct AtRefCrSocCreateResponse_Tag{
    UINT8  connectId;
    UINT8  accessMode;//0-DIRECT mode;1-PASSTHROUGH_MODE;2/3-buff mode
    UINT16 result;
}AtRefCrSocCreateResponse;//4 bytes

typedef struct AtRefCrSocketCloseReq_Tag{
    UINT8 connetId;
    UINT8 rsvd0;
    UINT16 rsvd1;
    UINT32 reqSource;
}AtRefCrSocketCloseReq;//8 bytes

typedef struct AtRefCrEncode_Tag
{
    UINT8 sendFormat;
    UINT8 recvFormat;
    UINT8 rsv[2];
...
typedef struct AtRefCrKeepalive_Tag
{
    UINT8 keepAliveEnable;
    UINT8 keepCount;
    UINT8 rsv[2];
    UINT16 keepIdle;
    UINT16 keepInterval;
...
typedef enum AtRefCrStateTransType_Tag
{
    REF_CR_STATE_TRANS_TYPE_INVALID = 0,
    REF_CR_STATE_TRANS_TYPE_UDP,
    REF_CR_STATE_TRANS_TYPE_TCP,
}AtRefCrStateTransType;

typedef struct AtRefCfgSetReq_Tag
{
    UINT8 connectionId;
    UINT8 paramType;  //AtRefCrCfgType
    UINT8 rsv[2];
    union
    {
        UINT8 cid;
        AtRefCrEncode encoding;
        UINT8 sendTimeout;
        UINT8 autoFree;
        UINT16 sendBuf;
        UINT16 recvBuf;
...
typedef struct AtRefCrCfgSetRsp_Tag
{
    UINT16 result;
    UINT8  rsv[2];
}AtRefCrCfgSetRsp;
...
typedef struct AtRefCrConnState_Tag
{
    ...
    CHAR    *pRemoteIp;/*save remote ip addr, max len = AT_SOC_MAX_REF_URL_IPADDR_LEN.
                        *Must be freed after used if not PNULL !!!
                        */
}AtRefCrConnState;/*8 Bytes*/

typedef UINT32 AtRefCrGetStateReq;

typedef struct AtRefCrGetStateRsp_Tag
{
    UINT16  result;
    UINT8   rsv[2];

    AtRefCrConnState connState[REF_CR_MAX_CONNECTION_NUM];
}AtRefCrGetStateRsp;

typedef struct AtRefCrGetSAckReq_Tag
{
    UINT8 connectionId;
    UINT8  rsv[3];
}AtRefCrGetSAckReq;

typedef struct AtRefCrGetSAckRsp_Tag
{
    UINT16 result;
    UINT8  connectionId;
    UINT8  rsv;
...
typedef AtRefCrGetSAckRsp AtRefCrGetSAckInfo;

typedef struct AtRefCrReadBufInfoReq_Tag
{
    UINT8  connectionId;
    UINT8  rsv[3];
}AtRefCrReadBufInfoReq;

typedef struct AtRefCrReadBufInfoRsp_Tag
{
    UINT16 result;
    UINT8 connectionId;
    UINT8 transType;  /* AtCmfRefStateTransType */

    union
    {
        UINT32 unReadDataLen;
        UINT32 unReadDataPacket;
    }u;
}AtRefCrReadBufInfoRsp;

typedef AtRefCrReadBufInfoRsp AtRefCrBufInfo;

typedef struct AtRefCrReadBufDataReq_Tag
{
    UINT8  connectionId;
    UINT8  rsv;
    UINT16 reqDataLenOrPacketCnt;

    UINT32 reqHandle;
}AtRefCrReadBufDataReq;

typedef struct AtRefCrReadDataUdpInfo_Tag
{
    UINT16 readedPackCnt;
    UINT16 remainPackCnt; /* packet count still in buffer */
}AtRefCrReadDataUdpInfo;

typedef struct AtRefCrReadDataTcpInfo_Tag
{
    UINT16 readedLen;  /* total readed len */
    UINT16 remainLen;  /* bytes still in buffer */
}AtRefCrReadDataTcpInfo;

typedef struct AtRefCrReadBufDataRsp_Tag
{
    UINT16 result;
    UINT8 connectionId;
    UINT8 transType;  /* AtRefCrStateTransType */
    ...
    union
    {
        AtRefCrReadDataUdpInfo udpDataInfo;
        AtRefCrReadDataTcpInfo tcpDataInfo;
    }u;
    CmsSockMgrDataContext   *dataContext;
}AtRefCrReadBufDataRsp;

typedef AtRefCrReadBufDataRsp AtRefCrBufData;

typedef struct AtRefCrTckAckInd_Tag
{
    UINT8  connectionId;
    UINT8  rsv;
    UINT16 tcpAckLen;
...
typedef struct AtRefCrUdpBuffInd_Tag
{
    UINT8  connectionId;
    UINT8  rsv;
    UINT16 udpBuffCount;
...
typedef struct AtRefCrUdpDataInd_Tag
{
    UINT8 connectionId;
    UINT8 recvFormat;
    UINT8 rsv[2];
...
typedef struct AtRefCrTcpBuffInd_Tag
{
    UINT8  connectionId;
    UINT8  rsv;
    UINT16 rcvLen;
...
typedef struct AtRefCrTcpDataInd_Tag
{
    UINT8 connectionId;
    UINT8 recvFormat;
    UINT8 rsv[2];
...
typedef struct AtRefCrDropDataInd_Tag
{
    UINT8 connectionId;
    UINT8 rsv;
    UINT16 dropLen;
...
typedef struct AtRefCrConnErrInd_Tag
{
    UINT8 connectionId;
    UINT8 rsv;
    UINT16 errorCode;
...
typedef struct AtRefCrUdpSeqInd_Tag
{
    UINT8 connectionId;
    UINT8 seq;
    UINT8 ret;
...
typedef struct AtRefCrDataInd_Tag
{
    UINT8 connectionId;
    UINT8 recvFormat;
    UINT8 rsv[2];
 ...
 * 2> '+++'/DTR, transfer from 2 (pass throgh mode) to 0 (buffer mode)
*/
typedef struct AtRefCrSocSetDataAccModeReq_Tag{
    UINT8   connectId;
    UINT8   mode;       /* AtRefataAccessMode, 0-direct mode, 1-pass through mode, 2/3- buffer mode */
    UINT8   trgSrc;     /* trigger source, 0 - AT+MIPMODE, 1 - '+++'*/
    UINT8   resv0;

    UINT16  atHandler;  /* Request AT hander, in case of "+++", pass the old atHandler */
    UINT16  error;
}AtRefCrSocSetDataAccModeReq;//8 bytes


typedef struct AtRefCrSocSetDataAccModeCnf_Tag{
    UINT8   connectId;
    UINT8   setNewmode;     /* AtRefataAccessMode,  0-direct mode, 1-pass through mode, 2/3- buffer mode */
    BOOL    needChangeMode; /* If in connecting state and change the state manual, don't change AT state in conf process*/
    UINT8   trgSrc;         /* trigger source, 0 - AT+MIPMODE, 1 - '+++'*/

    UINT16  atHandler;
    UINT16  revd;

    AtRefCrReadBufDataRsp *pDlDataRemaind; //Used for sending remaind DL data
}AtRefCrSocSetDataAccModeCnf;//8 bytes



typedef struct AtRefCrSocGetDataAccModeReq_Tag{
    UINT8  connectId;
    BOOL   bAll;
    UINT16 resv1 ;
    UINT32 reqSource;
}AtRefCrSocGetDataAccModeReq;//8 bytes

typedef struct AtRefCrSocGetDataAccModeCnf_Tag{
    struct{
            UINT8 connectId :4; //0~5
            UINT8 mode      :4; //0~3
        }accModeInfo[AT_REFCR_SOCK_SUPPORT_CONNECT_MAX];//Index by connectId
    UINT8  validNum;
    UINT8  resv0;
}AtRefCrSocGetDataAccModeCnf;//8 bytes

typedef struct AtRefCrSocSendCnf_Tag{
    UINT8   primId;
    UINT8   connectId;
    UINT16  sendLen;
    UINT32  reqSource;
...
typedef struct AtRefCrSocCloseInd_Tag{
    UINT8  connectId;
    UINT8  resv0;
    UINT16 resv1;
...
typedef struct AtRefCrSocketCnf_Tag
{
    UINT32 errCode;
    union{
           /*AT+MIPSEND to send data*/
          AtRefCrSocSendCnf           sendSocDataCnfMsg;

          /*AT+MIPMODE to set*/
         AtRefCrSocSetDataAccModeCnf  setAccModeCnfMsg;

          /*AT+MIPMODE to get*/
         AtRefCrSocGetDataAccModeCnf  getAccModeCnfMsg;

    }body;

}AtRefCrSocketCnf;

typedef struct AtRefCrSocPriMgrContext_Tag
{
    UINT8                  connectId :4;      /*connectId 1~11*/
    UINT8                  pdpContextId :4;   /*pdp contextId from 0~15*/

    UINT8                  accessMode:2;     /*accessMode 0,1,2,3*/
    UINT8                  sockstate:3;
    UINT8                  resv0:3;
    ...
    AtRefSocServiceType    serviceType;  /*serviceType,0~1*/
    UINT8                  resv2;
    UINT16                 remotePort;

    INT32                  errorCode ;    /*record the sockerrno when erroc occured*/

    UINT32                 ulTotalLen;      /* todo increase in send interface, include send(eg: atRefSocketHandleSendReq()) and send pendingList(eg: atRefsocSendPendingListReq()) */
    UINT32                 ulTotalAckedLen; /* Get from SOCK_EVENT_CONN_UL_TOTAL_STATUS event */
...
void atSockEnableUlfc(UINT8 chanId);
void atSockDisableUlfc(UINT8 chanId);
void *atSockAllocUlfcMem(UINT32 wantedSize,UINT8 chanId);
AtRefSocketSendInfo *atSockAllocDataSendInfo();
AtRefSocketSendInfo *atSockFindDataSendInfo(UINT8 channelId);
void* atSockAllocSendInfoDataBody(UINT16 maxLen, UINT8 chanId);
void atSockAddNewDataSendInfoToList(AtRefSocketSendInfo *pSocketSndInfo);
void atSockRemoveDataSendInfoList(AtRefSocketSendInfo *pSocketSndInfo, BOOL freeData);
void atSockStartDataSendWaitTimer(AtRefSocketSendInfo *pSocketSndInfo);
void atSockStopDataSendWaitTimer(AtRefSocketSendInfo *pSocketSndInfo);
void atSockProcDataSendWaitTimerExpiry(OsaTimerExpiry *pTimerExpiry);

#ifdef FEATURE_PS_REF_CR_TCPIP_SOCK_AT_ENABLE

void atRefCrSocEventCallback(CmsSockMgrContext *mgrContext, CmsSockMgrEventType eventType, void *eventArg);
RefCrSocketCfg* atRefCrSocGetSocketCfg(UINT8 connectionId);
RefCrTcpHeartBeatCfg* atRefCrSocGetTcpHeartBeatCfg(UINT8 connectionId);
RefCrCfg* atRefCrSockGetCfg();
CmsSockMgrContext* atRefCrGetCmsMgrContext(UINT8 connectionId);
