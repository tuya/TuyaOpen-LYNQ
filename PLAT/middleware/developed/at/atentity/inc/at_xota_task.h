...
#define XOTA_RESPDESC_SET(des, buf, code, status, um)    \
        do{\
            memset(&des, 0, sizeof(XotaRespDesc_t));\
            des.respBuf    = (buf);\
            des.cmdCode    = (code);\
            des.pkgStatus  = (uint8_t*)(status);\
            des.u.respArgs = (uint8_t*)(um);\
        }while(0)
...
typedef struct
{
    ...
    uint32_t  hasOffs  :1;
    uint32_t  alignVal :3;
    uint32_t  rsvdBits :26;
    uint8_t   dlvrFlags; /* XotaDlvrFlags_e */
    uint8_t   crc8;
    uint16_t  pkgSn;
    uint32_t  pkgOffs;
...
typedef struct
{
    ...
    uint16_t  lastPkgSn;
    uint8_t   rsvd[3];
    uint8_t   pkgStatus;  /* XotaPkgStatus_e */
    uint8_t   otaState;   /* XotaOtaState_e */
    uint8_t   otaResult;
    uint32_t  errCode;    /* AtXotaErrCode_e or RefCrFotaErrCode_e */
}XotaQueryStatus_t;
...
typedef struct
{
    ...
    uint8_t   cmdCode;   /* XotaCmdCode_e */
    uint8_t   rsvd[3];
    uint8_t  *pkgStatus; /* g_xotaPkgStatus[] */
    union
    {
        uint8_t  *respArgs;
        union
        {
            ...
            uint8_t  *fwNameStr;
            uint8_t  *csumStr;
            XotaQueryStatus_t  *status;
        }cnf;
        union
        {
            XotaReadData_t  *rdData;
        }ind;
    }u;
}XotaRespDesc_t;

typedef int32_t (*xotaEncapRespFunc)(XotaRespDesc_t *desc);
...
typedef struct
{
    ...
    uint16_t  rsvd;
    uint8_t  *cmdName;
    xotaEncapRespFunc  encapCnfRespFn;
    xotaEncapRespFunc  encapIndRespFn;
}XotaAppConf_t;
