...
#else
#define CIS_ERRID_NO_NETWORK              8009
#endif
...
#if ONENET_AUTO_UPDATE_ENABLE || defined FEATURE_REF_AT_CR_ENABLE
...
typedef enum
{
    ...
    CREATE_SET,
    CREATE_OTHER,
}onenet_create_type_e;

typedef enum
{
    ONENET_PLATFORM_ONENET,//0
    ONENET_PLATFORM_CTWING,//1
    ONENET_PLATFORM_GEWU,//2
    ONENET_PLATFORM_OCEAN,//3
    ONENET_PLATFORM_OTHER=10,
}onenet_platform_e;

typedef enum
{
    ENDPOINT_PATTERN_NO,//0
    ENDPOINT_PATTERN_IMEI,//1 ct oc default
    ENDPOINT_PATTERN_IMEI_IMSI,//2 IMEI:IMSI onenet default
    ENDPOINT_PATTERN_URN_IMEI,//3 urn:imei:#### ct op
    ENDPOINT_PATTERN_URN_IMEI_IMSI,//4 urn:imei-imsi:####-### ct op
}onenet_endpoint_pattern_e;

typedef struct {
    BOOL bUsed;
    cis_oid_t objectId;
    cis_rid_t resouceId[ONENET_ATTR_NUM];
    cis_attrcount_t attrCount;
    UINT8 discoverrspResult;
} onenetObjResInfo;
#endif
...
typedef struct cisMIPLCfg_v2_Tag
{
    // 0x0
    CHAR ip[40];
    // 0x28
    ...
    UINT8 monitEnable:1;
    UINT8 nsmiEnable:1;
    UINT8 dtlsRenegEnable:1;
    UINT8 dtlsRenegTime:7;
    UINT8 platform:4;
    UINT8 pattern:4;
    // 0x54
    CHAR endpointname[63];
    UINT8 bindingMode:3;
    UINT8 updateSet:1;
    UINT8 reserved:7;
    // 0x94
...
typedef struct {
    ...
    BOOL bConnected;
    BOOL bRestoreFlag;
    UINT8 FotaState;//0:no fota; 1:fota; 2:ctwing fota reboot
    UINT32 onenetId;
    void *cis_context;
    cis_time_t lifeTime;
    cis_time_t timeout;
...
#define CIS_NVM_FILE_VERSION_4   4
typedef struct {
    //offset 0
    UINT32 bUsed:1;
    UINT32 bConnected:1;
...
typedef onenetRetentionContext_v4 onenetRetentionContext;
...
bool onenetDLPopout(uint8_t* dlType, uint32_t* mid);
...
INT32 onenetMonitConfig(UINT32 onenetId, BOOL monit_enable);
void onenetMonitGet(UINT32 onenetId, BOOL *monit_enable);
INT32 onenetNsmiConfig(UINT32 onenetId, BOOL nsmi_enable);
void onenetNsmiGet(UINT32 onenetId, BOOL *nsmi_enable);
...
INT32 onenetDTLSRenegConfig(UINT32 onenetId, BOOL dtls_reneg_enable, UINT8 dtls_reneg_time);
void onenetDTLSRenegGet(UINT32 onenetId, BOOL *dtls_reneg_enable, UINT8 * dtls_reneg_time);
bool onenetURCSendMode(void);
void onenetMIPCFGSet(UINT32 onenetId, uint8_t platform, uint8_t pattern, uint8_t bindingMode, char* epname);
void onenetUPDATESETCMDSet(UINT32 onenetId, uint8_t enable);
...
#ifdef FEATURE_REF_AT_CR_ENABLE
onenetObjResInfo *onenetGetObjResInsfo(void);
INT32 onenetObvRspValid(UINT32 onenetId, UINT32 msgid);
INT32 onenetDiscovRspValid(UINT32 onenetId, UINT32 msgid, BOOL bNormal);
#endif
