...
typedef enum AT_MHTTP_ERROR
{
    ...
    MHTTPAT_CFGPARAM_ERROR = 9001,                            //MHTTPCFG parameter error
}atMHttpError_e;
...
typedef struct {
    ...
    uint8_t eof:1;
    uint8_t isRange:1;
    uint8_t hasSentfirstChunkedBlock:1;
    uint8_t resve:4;
    CHAR* host;
    CHAR* path;
    CHAR* url;
...
bool mhttpRequestReq(UINT32 atHandle, uint8_t clientId, uint8_t* path, char* local_path);
bool mhttpCheckRequest(uint8_t clientId);
void mhttpSendChunkedContentReq(mhttpAtCfgParam_t* listNode, uint8_t clientId);
...
int mhttpHeadListFree(mhttpHeadNode_t **headList);
