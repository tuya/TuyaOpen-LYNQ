...
#define CR_MQTT_TX_BUF_DEFAULT        (4+128+8*1024)    /*header+topic+payload*/
#define CR_MQTT_RX_BUF_DEFAULT        (4+128+4*1024)    /*header+topic+payload*/
...
#define CR_MQTT_SUB_NODE_MAX       30
...
#define CR_MQTT_RSP_HEADER_LEN      96
#define CR_MQTT_RSP_PRINT_ONCE_LEN  500
#define CR_MQTT_SSL_ID_DEF          0xFF
...
typedef struct
{
    ...
    UINT16 inputFormat;
    UINT16 inputFormatTemp;  //if pub payload len is 0, the input data is string
    UINT16 outputFormat;
    UINT32 reconnTimes;
    ...
    CHAR *ca_key;
    CHAR *host_name;
    crMqttSubNodeInfo_T subNode[CR_MQTT_SUB_NODE_MAX];
...
int crMqttCheckPayLoad(char *payLoad, int payLoadLen);
