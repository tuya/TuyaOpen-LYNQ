...
#define AT_CMD_MAX_PENDING_STR_LEN      9216        //MAX pending 9KB for cr mqtt
#else
...
typedef enum AtcState_enum
{
    ...
    */
    ...
    ATC_REF_CR_SOCKET_SEND_DATA_STATE,
    ...
    ATC_MHTTP_INPUT_STATE,
    ATC_REF_CR_MQTT_PUB_DATA_STATE,
    ATC_MSSL_PASSTHROUGH_STATE,
    ATC_MSSL_INPUT_STATE,
    ATC_REF_CR_FILE_WRITE_DATA_STATE,

    ATC_MFTP_PASSTHROUGH_STATE,
    ATC_MFTP_INPUT_STATE,
...
typedef struct AtChanMiscellaneous_Tag
{
    ...
    UINT32  endStartMux  : 1;   /* whether need to start cmux mode, after AT proc done */
    UINT32  rsvd         : 11;
    UINT32  baudSaveFlag : 1;   /* when when endSetbaud == TRUE */
    UINT32  poweroffMode : 8;   /* when endPowerOff == TRUE, pass to power off action */
    UINT32  cmuxWorkMode : 8;   /* when endStartMux == TRUE, pass to start cmux action */
...
typedef struct AtChanUrcInfo_Tag
{
     ...
     * Example:
     * a) Bit0-1: pUrcHead, 0 - other URC, 1 - SMS URC, 2 - RING URC.
    */
    UINT32      smsUrcBitmap;
...
/*
*/
CmsRetId atcEndStartCmuxMode(UINT8 chanId, UINT32 mode, UINT8 hasNewBaud, UINT32 baudrate, UINT32 frameFormat);
