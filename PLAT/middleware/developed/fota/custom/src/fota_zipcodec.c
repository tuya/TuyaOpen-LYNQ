...
int32_t FOTA_encodeLzma(FotaBufferDesc_t *outBuf, FotaBufferDesc_t *inBuf)
{
    ...
    FOTA_BUF_DESC_APPEND(outBuf, size);
#else
    FOTA_TRACE(LOG_DEBUG,"lzma encode: unsupp err!\n");
#endif

    return ret;
}

PLAT_BL_CIRAM_FLASH_TEXT int32_t FOTA_decodeLzma(FotaBufferDesc_t *outBuf, FotaBufferDesc_t *inBuf)
{
    int32_t   ret = FOTA_EOK;
#if FEATURE_LZMA_ENABLE
    unsigned int  size = 0;
    ...
    FOTA_BUF_DESC_APPEND(outBuf, size);
#else
    FOTA_TRACE(LOG_DEBUG,"lzma decode: unsupp err!\n");
#endif
