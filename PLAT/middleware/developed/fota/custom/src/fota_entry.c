...
AP_PLAT_COMMON_BSS static uint8_t          gFotaIsUsbUrcEn = 0;
...
static void FotaUsbUrcEn(uint8_t vcomNum)
{
    ...
    if(0 == BSP_UsbInit())
    {
        FOTA_TRACE(LOG_DEBUG, "init USB succ! \n");
    }
    else
    {
        FOTA_TRACE(LOG_DEBUG, "init USB fail! \n");
    }
    ...
    while(1)/*loop for enum done*/
    {
        ...
        if(vcomx_isconnect(vcomNum) == 0)
        {
            ...
            if(loopCnt == 0)
            {
               FOTA_TRACE(LOG_DEBUG, "loop USB ENUM fail! \n");
               break;
            }
        }
        else
        {
            FOTA_TRACE(LOG_DEBUG, "loop USB ENUM succ %d! \n", loopCnt);
            break;
        }
    }
...
void FotaProcedure(void)
{
    ...
    if(hasNewDfu == 1)//end
    {
        ...
        if(gFotaIsUsbUrcEn == 1)
        {
            ...
            {
                FOTA_TRACE(LOG_DEBUG, "deinit USB URC fail! \n");
            }
        #endif
        }
