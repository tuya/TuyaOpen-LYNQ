...
int32_t FOTA_getHlsWorkSpaceBuf(FotaBufferDesc_t *desc)
{
    ...
    FOTA_BUF_DESC_SET(desc, (uint8_t*)((uint32_t)&_fota_mux_buf_start + FOTA_PMB_HLS_WORK_SPACE_OFFS), FOTA_PMB_HLS_WORK_SPACE_SIZE, 0);
#endif
#endif
    return FOTA_EOK;
}
...
#if (FOTA_PRESET_RAM_ENABLE == 1)
#ifdef FEATURE_FOTA_HLS_ENABLE
    FOTA_BUF_DESC_SET(desc, (uint8_t*)((uint32_t)&_fota_mux_buf_start + FOTA_PMB_HLS_UNZIP_BASE_OFFS), FOTA_PMB_HLS_UNZIP_BASE_SIZE, 0);
#endif
#endif
    return FOTA_EOK;
}
...
#if (FOTA_PRESET_RAM_ENABLE == 1)
#ifdef FEATURE_FOTA_HLS_ENABLE
    FOTA_BUF_DESC_SET(desc, (uint8_t*)((uint32_t)&_fota_mux_buf_start + FOTA_PMB_HLS_UNZIP_NEXT_OFFS), FOTA_PMB_HLS_UNZIP_NEXT_SIZE, 0);
#endif
#endif
    return FOTA_EOK;
}
...
#if (FOTA_PRESET_RAM_ENABLE == 1)
#ifdef FEATURE_FOTA_HLS_ENABLE
    FOTA_BUF_DESC_SET(desc, (uint8_t*)((uint32_t)&_fota_mux_buf_start + FOTA_PMB_HLS_BASE_PROF_OFFS), FOTA_PMB_HLS_BASE_PROF_SIZE, 0);
#endif
#endif
    return FOTA_EOK;
}
...
{
#ifdef FEATURE_FOTA_HLS_ENABLE
    return FOTA_PMB_HLS_BASE_PROF_SIZE;
#else
    return 0;
#endif
}
...
int32_t FOTA_chksumFlashData(uint8_t/* FotaChksumAlgo_e */ algo, uint32_t zid, uint32_t offset, uint32_t size, uint8_t *hash, uint32_t isLast, buf_handle_callback buf_handle_cb)
{
    ...
    if(!sha256.input)
    {
        ECPLAT_PRINTF(UNILOG_FOTA, FOTA_CHKSUM_FLASH_2, P_ERROR, "sha256sum: alloc buf(%d) fail!\n", allocSize);
        return FOTA_EMALLOC;
    }
    ...
    for(loop = 0; loop < size; loop += allocSize)
    {
        ...
        {
            ECPLAT_PRINTF(UNILOG_FOTA, FOTA_CHKSUM_FLASH_3, P_WARNING, "zid(%d) flash read fail!\n", zid);
            errCode = FOTA_EFLREAD;
            goto CHKSUM_FLASH_END;
        }
        ...
        if(FOTA_EOK != (errCode = fotaCalcChksum(algo, &sha256)))
        {
            ECPLAT_PRINTF(UNILOG_FOTA, FOTA_CHKSUM_FLASH_4, P_ERROR, "zid(%d) flash cs-calc fail! errCode = %d\n", zid, errCode);
            errCode = FOTA_ECHKSUM;
            goto CHKSUM_FLASH_END;
        }
    }
...
int32_t FOTA_chksumBufData(uint8_t/* FotaChksumAlgo_e */ algo, uint8_t *buf, uint32_t size, uint8_t *hash, uint32_t isLast, buf_handle_callback buf_handle_cb)
{
    ...
    if(FOTA_EOK != fotaCalcChksum(algo, &sha256))
    {
