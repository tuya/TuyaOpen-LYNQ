...
#define FOTA_NVM_SECTOR_SIZE           FOTA_SECTOR_BUF_SIZE
#define FOTA_NVM_BLOCK_SIZE            FOTA_BLOCK_BUF_SIZE
...
//#define FOTA_NVM_APP2_LOAD_ADDR            (APP2_FLASH_LOAD_ADDR & (~FOTA_NVM_A2AP_XIP_ADDR))
//#define FOTA_NVM_APP2_LOAD_SIZE            (APP2_FLASH_LOAD_SIZE)
#endif
...
int32_t fotaNvmFsOpen(const char *fname, const char* mode)
{
    ...
    ECPLAT_PRINTF(UNILOG_FOTA, FOTA_NVM_FS_OPEN_1, P_INFO, "fs: open %s with mode'%s' @flag(%d)\n",
                                                            fname, mode, gFotaLfsHndl.openFlag);
...
int32_t fotaNvmFsClose(int32_t fd)
{
    ...
    if(gFotaLfsHndl.openFlag == 1)
    {
        ...
        memset(&gFotaLfsHndl.fp, 0, sizeof(lfs_file_t));
    }
...
{
    if(gFotaLfsHndl.openFlag)
    {
        ECPLAT_PRINTF(UNILOG_FOTA, FOTA_NVM_FS_REMV_1, P_WARNING, "fs: can not remv %s err while in using!\n", fname);
        return FOTA_EOK;
    }
...
int32_t fotaNvmFsOpenBrandNew(const char *fname, const char* mode)
{
    if(gFotaLfsHndl.openFlag)
    {
        ECPLAT_PRINTF(UNILOG_FOTA, FOTA_NVM_FS_OPEN_BRANDNEW_1, P_WARNING, "fs: %s already opened! force to close it!\n", fname);
        fotaNvmFsClose(gFotaLfsHndl.fd);
    }
    ...
    return fotaNvmFsOpen(fname, mode);
}
#endif
...
static int32_t fotaNvmNfsWrite(uint32_t zid, uint32_t offset, uint8_t *buf, uint32_t bufLen)
{
    ...
    if(offset >= (maxOffs = gFotaNvmZoneMan.zone[zid].size - gFotaNvmZoneMan.zone[zid].overhead))
    {
...
static int32_t fotaNvmAdjustZoneSize(FotaDefAdjZoneSize_t *adjZone)
{
    ...
    if(orginSize != gFotaNvmZoneMan.zone[adjZone->zid].size)
    {
        ECPLAT_PRINTF(UNILOG_FOTA, FOTA_NVM_ADJ_ZONESZ_2, P_INFO, "adj zone(%d): size %d --> %d\n",
                                 adjZone->zid, orginSize, gFotaNvmZoneMan.zone[adjZone->zid].size);
    }
    else
    {
...
static int32_t fotaNvmCheckDeltaState(FotaDefChkDeltaState_t *chkDelta)
{
...
#else
    if(parh.parLen > fotaNvmGetDeltaSize(1))
    {
        chkDelta->state = FOTA_DCS_DELTA_PARTIAL;
        ECPLAT_PRINTF(UNILOG_FOTA, FOTA_CHK_DELTA_4, P_SIG, "delta: size(%d) overflow! max size(%d)\n", parh.parLen, fotaNvmGetDeltaSize(1));
        return FOTA_EPARSZ;
    }
#endif
    ...
    if(pauh.curveType == 1)
    {
        ...
        if(FOTA_EOK != retCode)
        {
            ECPLAT_PRINTF(UNILOG_FOTA, FOTA_CHK_DELTA_5, P_ERROR, "delta: sha224 hash failed(%d)!\n", retCode);
            return retCode;
        }
    }
    else
    ...
    chkDelta->state = FOTA_DCS_DELTA_INVALID;
    memcpy(chkDelta->hash, parh.parHash, FOTA_SHA256_HASH_LEN);
...
{
    if(isIdent->size == 0) return FOTA_EOK;
...
PLAT_BL_CIRAM_FLASH_TEXT int32_t fotaNvmInit(void)
{
    ...
    int32_t fd = fotaNvmFsOpenBrandNew(FOTA_DELTA_PAR_NAME, "rw+");
#endif
    if(fd < 0)
    {
...
uint8_t fotaNvmDoesDeltaExist(void)
{
    uint8_t  buf[FOTA_BUF_SIZE_16] = {0};

    fotaNvmRead(FOTA_NVM_ZONE_DELTA, 0, buf, FOTA_BUF_SIZE_16);

    return (FOTA_CHECK_AUTH_MAGIC(buf) || FOTA_CHECK_PAR_MAGIC(buf)) ? 1 : 0;
}
...
int32_t fotaNvmVerifyDelta(uint32_t zid, uint8_t *hash, uint32_t pkgSize, uint32_t *deltaState)
{
    ...
    if(hash)
    {
        memcpy(hash, chkDelta.hash, FOTA_SHA256_HASH_LEN);
    }
