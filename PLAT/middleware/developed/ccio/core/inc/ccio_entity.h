...
typedef enum _EPAT_CcioEntStatusFlagsTag
{
    ...
    CCIO_ESF_SET_AUDIO_ENTITY,
    CCIO_ESF_UNSET_AUDIO_ENTITY,
     ...
     */
    CCIO_ESF_CUST_RSVD_END = 0x0E5FCFFF,

    CCIO_ESF_UNDEF = 0x0E5FFFFF,
    CCIO_ESF_END = CCIO_ESF_UNDEF
...
typedef struct
{
    ...
    uint8_t    rsvd[3];
    void      *chent;
}CcioEsaUnsetEntity_t;

/* CCIO_ESF_EOS_RESET */
typedef struct
{
    void   *eosEnt;           /* EthosEntity_t */
    CcioBufDesc_t  resetMsg;  /* SpicNetResetMsg_t */
}CcioEsaEosReset_t;
...
{
    void   *eosEnt;           /* EthosEntity_t */
    CcioBufDesc_t  initMsg;   /* SpicNetInitialMsg_t */
}CcioEsaEosInit_t;
...
/**
 * @brief ccioFindEntity(uint32_t chanId)
 ...
 */
CcioEntity_t* ccioFindEntity(uint32_t chanId);
