...
typedef enum
{
    ...
    CMUX_DLC_CONNECTING,  /* unused yet! */
    CMUX_DLC_CONNECTED,
    CMUX_DLC_DISCONNECTING,
    CMUX_DLC_FLOW_STOPPED /* unused yet! */
}CmuxDlcFsm_e;
...
typedef struct CmuxDlcCtxMan
{
    ...
    SerialEntity_t   *serlEnt;     /* the serialEntity the cmuxCtx is belonged to */
...
typedef struct
{
    ...
    uint32_t  hasN1:1;
    uint32_t  rsvd :7;
    uint32_t  n1   :16; /* mtu (maximum payload size). */
 ...
 *----------------------------------------------------------------------------*/
/**
 * @brief cmuxInitDlcComnConf(void)
 * @details initialize cmux common config parameters
 *
 * @return VOID.
 */
void cmuxInitDlcComnConf(void);
...
/**
 * @brief cmuxFrameOutput(CcioDevice_t *chdev, DlPduBlock_t *dlpdu, void *extras)
 * @details cmux frame output
 *
 * @param chdev    The device used to output cmux frame
 * @param dlpdu    cmux frame descriptor
 * @param extras   extra essential info for output
 * @return 0 succ; < 0 failure with errno.
 */
int32_t cmuxFrameOutput(CcioDevice_t *chdev, DlPduBlock_t *dlpdu, void *extras);

/**
 * @brief cmuxDlcSetFsm(CmuxDlcEntity_t *dlcEnt, uint8_t newFsm)
 * @details set dlc new fsm
 *
 * @param dlcEnt    The dlc to be set new fsm
 * @param newFsm    new fsm
 */
void cmuxDlcSetFsm(CmuxDlcEntity_t *dlcEnt, uint8_t newFsm);
