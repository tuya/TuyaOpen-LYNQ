...
char cmsDecToDecStr(char *s, unsigned value);
 ...
 * "\r12\n" -> {0x0d, 0x31, 0x32, 0xa}
*/
INT32 cmsChrToUintWithEscapeCharacter(UINT8 *pOutput, UINT32 outBufSize, CHAR *pCharStr);
...
/******************************************************************************
 * psImaImiReq
 * Description: PS IMA IMI request, reques/access ims service
 * input: UINT16    srcHdr;
 *        UINT8     sgId;       // ImaSgIdEnum
 *        UINT16    primId;     // prim ID
 *        UINT16    primSize;   //prim body size
 *        void      *primBody;  //prim body
 * output: void
 * Comment:
 *  1> Before send the signal, need to check whether PS/IMS task is ready, if not, need to
 *      start PS/IMS firstly
 *  2> This API could only be called in CMS task
******************************************************************************/
void psImaImiReq(UINT16 srcHdr, UINT8 sgId, UINT16 primId, UINT16 primSize, void *primBody);
#endif

BOOL cmsRemoveQuotes(CHAR **ppCharStr);

UINT32 cmsParseConsecutiveAtComd(UINT8 **ppOutput, CHAR *pCharStr);
