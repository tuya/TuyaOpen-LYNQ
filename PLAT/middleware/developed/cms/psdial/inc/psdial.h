...
typedef struct PsDialContext_Tag
{
    ...
    UINT32      logDumpRptMode  : 2;
    UINT32      logDumpRptChanId: 6;

    CmiSimImsiStr   ueImsi;             //20 bytes
    NmIpv6Prefix    *pIp6Prefix;        //used for remap case
    osTimerId_t     logDumpTimerId;     //used for PS info dump to flash guard timer
...
void psDialProcPsDumpTimerExpiry(OsaTimerExpiry *pTimerExpiry);
#endif
