...
#if defined FEATURE_REF_AT_CR_ENABLE
#define EC_NV_DATA_LEN                  256       /* not suitable to put here, -TBD */

/*-|--imei--|--sn--|--imei lock flag--|--sn lock flag--|--local mac--|--local mac crc--|--local mac lock flag--|--peer mac--|--peer mac crc--|--peer mac lock flag--|--cr hw ver--|--cr pid--|--cr pid flag--|--cr precision--|--cr nearbtsen--|-----*/
/* | 32byte |32byte|	  16byte	  | 	 16byte    |	 16byte  |	   8byte	   |		 8byte		   |	16byte	|	   8byte	 |		  8byte 		|	 32byte   |  32byte  |	   8byte	 |		2byte	  | 	  2byte    |	 */
#define EC_NV_DATA_CR_HW_VER_LEN         32
#define EC_NV_DATA_CR_PID_LEN            32
#define EC_NV_DATA_CR_PID_FLAG_LEN       8
#define EC_NV_DATA_CR_PRECISION_LEN       2
#define EC_NV_DATA_CR_NEARBTSEN_LEN       2

#define EC_NV_DATA_CR_HW_VER_OFFSET        160
#define EC_NV_DATA_CR_PID_OFFSET           192
#define EC_NV_DATA_CR_PID_FLAG_OFFSET      224
#define EC_NV_DATA_CR_PRECISION_OFFSET     232
#define EC_NV_DATA_CR_NEARBTSEN_OFFSET     234
...
#define SUPPORT_MAX_FREQ_NUM    32 //== CMI_DEV_SUPPORT_MAX_FREQ_NUM
...
#if defined FEATURE_REF_AT_CR_ENABLE
CmsRetId appGetCrHwVerSync(CHAR* hwVer);
BOOL appSetCrHwVerSync(CHAR* hwVer);
CmsRetId appGetCrPidFlagSync(CHAR* pidFlag);
CmsRetId appGetCrPidSync(CHAR* pid);
BOOL appSetCrPidSync(CHAR* pid);
BOOL appSetCrPidCleanSync(void);
CmsRetId appGetCrPrecisionSync(INT8* precision);
BOOL appSetCrPrecisionSync(INT8* precision);
CmsRetId appGetCrNearbtsenSync(INT8* nearbtsen);
BOOL appSetCrNearbtsenSync(INT8* nearbtsen);
...
/**
  \fn          appUSATGeoLocReqRspSync
  \brief       Send cmi request to fill terminal response for Geographical Location Request proactive command to SIM
  \param[in]   UINT8 generalResult, general result referred to CmiSimUsatGeneralResult
  \param[in]   BOOL addiMeProblemPresent, additional ME problem present
  \param[in]   UINT8 addiMeProblem, additional ME problem, referred to CmiSimUsatAddiInfoMeProblem
  \returns     CmsRetId
  \NOTE:
*/
CmsRetId appUSATGeoLocReqRspSync(UINT8 generalResult,
                                           BOOL addiMeProblemPresent,
                                           UINT8 addiMeProblem);

/**
  \fn          appUSATGeoLocReportReqSync
  \brief       Send cmi request to fill Envelope cmd (Geographical Location Reporting) to SIM
  \param[in]   UINT8 gadShapeLen, the length of GAD shape
  \param[in]   UINT8 *pGadShape, the pointer to GadShape
  \param[in]   UINT8 velocityLen, the length of velocity
  \param[in]   UINT8 *pVelocity, the pointer to velocity
  \param[in]   UINT8 nmeaSentenceLen, the length of NMEA Sentence
  \param[in]   UINT8 *pNmeaSentence, the pointer to NMEA Sentence
  \returns     CmsRetId
  \NOTE:
*/
CmsRetId appUSATGeoLocReportReqSync(UINT8 gadShapeLen,
                                              UINT8 *pGadShape,
                                              UINT8 velocityLen,
                                              UINT8 *pVelocity,
...
/**
  \fn          appUSATTerminalRspSync
  \brief       Send cmi request with terminal response data to SIM
  \param[in]   UINT8 trDataLen, the length of termianl response data
  \param[in]   UINT8 *pTrData, the pointer to the terminal resposne data, encoded data structure: command details+device identities+result+...
  \returns     CmsRetId
  \NOTE:       This api is used to reponse for PS event: PS_URC_ID_USAT_PROACT_CMD
*/
 CmsRetId appUSATTerminalRspSync(UINT8 trDataLen, UINT8 *pTrData);

/**
  \fn          appUSATEnvelopeCmdReqSync
  \brief       Send cmi request with Envelope Cmd (e.g. Menu selction) data to SIM
  \param[in]   UINT8 cmdDataLen, the length of command data
  \param[in]   UINT8 *pCmdData, the pointer to the command data, e.g. encoded data of Menu selection structure:
  \            Menu selection tag+Length+command details+Item identities+help request
  \returns     CmsRetId
  \NOTE:
*/
CmsRetId appUSATEnvelopeCmdReqSync(UINT8 cmdDataLen, UINT8 *pCmdData);
...
/**
  \fn          appSetCCWASync
  \brief       Set call waiting parameters
  \param[in]   UINT8 mode, call waiting feature mode, 0-disable, 1-enable, 2-query status
  \param[out]  UINT8 *pStatus, the pointer to status when mode is query status, 0-not active, 1-active
  \returns     CmsRetId
  \NOTE:       refer to AT+CCWA=,<mode>
*/
CmsRetId appSetCCWASync(UINT8 mode, UINT8 *pStatus);
...
/**
  ...
  \returns     CmsRetId
*/
void appSendMtSmsRsp(UINT8 smsId, BOOL bRcvOk, BOOL bSmsOverIp);
...
/**
  \fn          appSmsSetPrefMsgStorageSync
  \brief       selects memory storages <mem1>, <mem2> and <mem3> to be used for reading, writing, etc. +CPMS
  \param[in]   PsilSmsStoreMemType mem1, PsilSmsStoreMemType mem2, PsilSmsStoreMemType mem3
  \returns     CmsRetId
*/
CmsRetId appSmsSetPrefMsgStorageSync(PsilSmsStoreMemType mem1, PsilSmsStoreMemType mem2, PsilSmsStoreMemType mem3);
...
/**
  \fn          appSetCscaSync
  \brief       updates the SMSC address, +CSCA
  \param[in]   const CHAR *addrStr, RP SC address Address-Value field in string format
  \param[in]   PsilSmsTypeOfAddress toaOctType, RP SC address Type-of-Address
  \returns     CmsRetId
*/
CmsRetId appSetSmscSync(const CHAR *addrStr, PsilSmsTypeOfAddress toaOctType);

/**
  \fn          appGetSmscSync
  \brief       get the SMSC address, +CSCA
  \param[in]   CmiSmsAddressInfo *pScAddressinfo
  \returns     CmsRetId
*/
CmsRetId appGetSmscSync(CmiSmsAddressInfo *pScAddressinfo);

/**
  \fn          appSmsSetMessageFormatSync
  \brief       set message format +CMGF
  \param[in]   PsilSmsFormatMode   messageFormat
  \returns     CmsRetId
*/
CmsRetId appSmsSetMessageFormatSync(PsilSmsFormatMode messageFormat);

/**
  \fn          appSmsGetMessageFormatSync
  \brief       get message format +CMGF
  \param[in]   PsilSmsFormatMode &pMessageFormat
  \returns     CmsRetId
*/
CmsRetId appSmsGetMessageFormatSync(PsilSmsFormatMode *pMessageFormat);

/**
  \fn          appSmsSetNewMsgIndCfgSync
  \brief       get New Message Indications +CNMI
  \param[in]   MWNvmCfgSetCNMIParam *pCnmiParam
  \returns     CmsRetId
*/
CmsRetId appSmsSetNewMsgIndCfgSync(MWNvmCfgSetCNMIParam *pCnmiParam);

/**
  \fn          appSmsGetNewMsgIndCfgSync
  \brief       set New Message Indications +CNMI
  \param[in]   MWNvmCfgCNMIParam *pCnmiParam
  \returns     CmsRetId
*/
CmsRetId appSmsGetNewMsgIndCfgSync(MWNvmCfgCNMIParam *pCnmiParam);
