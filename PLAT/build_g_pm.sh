...
export PROJECT_NAME=tuyaos_adapter
...
export GCCLIB_PATH=`realpath "../../toolchain/gcc-arm-none-eabi-10-2020-q4-major"`
...
export BUILD_PRO_TARGET=L511G_Y7PM
...
# OEM tools may be checked out without execute bit
if [ -d "./tools" ]; then
...
build_lib()
{
	...
	# 判断是否存在 openlib/$BUILD_PRO_TARGET/bin
	if [ -d "openlib/$BUILD_PRO_TARGET/bin" ]; then
		mkdir -p gccout/$OUTPUT_NAME/ap/bootloader
		cp -rf openlib/$BUILD_PRO_TARGET/bin/* gccout/$OUTPUT_NAME/ap/bootloader/
	else
	...
	# Local toolchain often fails bincompress → no ap_bootloader.bin; fall back to OEM package image
	if [ ! -f "./gccout/$OUTPUT_NAME/ap/bootloader/ap_bootloader.bin" ]; then
		BL_FALLBACK="./L511G_Y7PM_download/ap_bootloader.bin"
		if [ ! -f "$BL_FALLBACK" ]; then
			BL_FALLBACK="./L511G_Y7PVM_download/ap_bootloader.bin"
		fi
		if [ -f "$BL_FALLBACK" ]; then
			echo "WARN: bootloader bincompress missing .bin, use $BL_FALLBACK"
			mkdir -p "./gccout/$OUTPUT_NAME/ap/bootloader"
			cp -f "$BL_FALLBACK" "./gccout/$OUTPUT_NAME/ap/bootloader/ap_bootloader.bin"
		else
			echo "ERROR: ap_bootloader.bin missing after bootloader build"
			exit 1
		fi
	fi
	...
	# Force full unilog DB regen (stale .pp skips LogPrePass / driver log IDs)
	if [ -d "./gccout/$OUTPUT_NAME/ap/$PROJECT_NAME" ]; then
		find "./gccout/$OUTPUT_NAME/ap/$PROJECT_NAME" -name '*.pp' -delete
		rm -f "./gccout/$OUTPUT_NAME/ap/$PROJECT_NAME/debug_log_ap.h" \
		  "./gccout/$OUTPUT_NAME/ap/$PROJECT_NAME/dbversion.h"
	fi
	...
	if [ $? -ne 0 ]; then
		echo "build-unilog failed"
		exit 1
	fi
	# copy log file
	if [ ! -f "./gccout/$OUTPUT_NAME/ap/$PROJECT_NAME/debug_log_ap.h" ]; then
		echo "missing debug_log_ap.h after build-unilog"
		exit 1
	fi
	...
	mkdir -p ../../../../../../gccout/$OUTPUT_NAME/ap
	make -$JOBNUMBER tuyaos_adapter TYPE=$CHIP_TYPE TARGET=$BOARD_NAME V=$VERBOSE EUTRAN_MODE=$EUTRAN_MODE PROJECT=$PROJECT_NAME CORE=ap BUILD_UNILOG=true BUILD_CUST=$CUST SDK=true MANUFACTURER=$MANUFACTURER AUDIO_PLAY_ORIGINAL_MODE_ENABLE=0 | tee ../../../../../../gccout/$OUTPUT_NAME/ap/outbuildlog.txt
	if [ ${PIPESTATUS[0]} -ne 0 ]; then
		echo "tuyaos_adapter build failed"
		exit 1
	fi
...
if [ $# -ge 1 ]; then
    if [ "$1" = "clean" ]; then
		...
		echo "unsupport build unilog"
	elif [ "$1" = "unilog_clean" ]; then
		echo "unsupport build unilog_clean"
	elif [ "$1" = "lib" ]; then
		build_lib
	else
