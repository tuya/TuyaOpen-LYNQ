 ...
 *****************************************************************************
******************************************************************************/
#define SUPPORT_MAX_FREQ_NUM            32
...
#define CEMM_EHPLMN_LIST_GROUP_MAX_NUM  4
#define CEMM_EHPLMN_LIST_ITEM_MAX_NUM   8
...
#define U8ToU32(input) (((UINT32)(input)[0] << 24) ^ \
                       ((UINT32)(input)[1] << 16) ^ \
                       ((UINT32)(input)[2] <<  8) ^ \
                       ((UINT32)(input)[3]))

#define U32ToU8(output, input) { (output)[0] = (UINT8)((input) >> 24); \
                                (output)[1] = (UINT8)((input) >> 16); \
                                (output)[2] = (UINT8)((input) >>  8); \
                                (output)[3] = (UINT8)(input); }
...
/******************************************************************************
 * Which IMS solution used
******************************************************************************/
typedef enum PsImsSolutionType_Tag
{
    PS_NO_IMS_SOLUTION  = 0,
    PS_EIMS_SOLUTION
}PsImsSolutionType;
...
typedef enum _SIG_EPAT_CCM_CFG_UE_SERVICE
{
    ...
    NON_EPS         = 1,
    EPS_AND_NON_EPS = 2,
    EMC_SERVICE     = 3,    /* request for emergency attach */
    NON_SERVICE
}UeService;
...
/******************************************************************************
 * VoiceDomainPreference(for AT+CEVDP <setting>)
...
typedef enum VoiceDomainPreference_Tag
{
    ...
    IMS_PS_VOICE_PREFERRED_CS_VOICE_SECONDARY = 3,
    IMS_PS_VOICE_ONLY                         = 4
