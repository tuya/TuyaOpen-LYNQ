...
typedef enum
{
    PSRAM_HYBD_VOTE_UART = 0,
    PSRAM_HYBD_VOTE_SPI,
	// add enum by customer

    PSRAM_HYBD_MAXIMUM = 15 
}slpManPsramHybdVote_e;
#endif
...
#if defined TYPE_EC718M
#define PSRAM_HYBD_VOTE_MAX_NUM 16
#endif
...
typedef enum
{
    ...
    SLP_CALLBACK_VPU_MODULE,            /**< sleep callback of VPU module */
#ifdef TYPE_EC718M
    SLP_CALLBACK_CAN_MODULE,            /**< sleep callback of CAN module */
#endif
    SLP_CALLBACK_ATCMD_MODULE,          /**< sleep callback of ATCMD module */
    SLP_CALLBACK_PSRAM_MODULE,          /**< sleep callback of PSRAM module */
...
typedef enum
{
    ...
    SLP_VOTE_VPU,                       /**< vote module: VPU */
#ifdef TYPE_EC718M
    SLP_VOTE_CAN,                       /**< vote module: CAN */
#endif
    SLP_VOTE_MAX_NUM,
...
typedef struct
{
    uint16_t psram_hybd_bitmap;
    uint8_t vote_counter[PSRAM_HYBD_VOTE_MAX_NUM];
}slpManPsramHybdVote_t;
#endif
...
* @param   idx: 0: apwm0  1: apwm1  2: apwm2
           ...
           slpManSetAPwmCfg(0, APWM_MAX_PERIOD_1024MS, 30, 80);  //pull high at position 307ms, and pull low at position 819ms
* @return null
* @note 
*/
...
*             718 can keep dcxo output in sleep state, but 716 only output dcxo when active
...
/**
* @brief void slpMan40KOutputEn(bool en);
* @details: to enable 40K output send out from pad: CLK26M_REF. (716 can not)
*             718 can keep dcxo output in all sleep state
* @param   true: enable output  false: disable output
* @return null
* @note 
*/
void slpMan40KOutputEn(bool en);
#endif
/**
...
/**
* @brief void slpManVoteSetDozeEnable(bool en)
* @details  allow doze state or not temporary in application layer.
            There is also a doze switch in at+ecpcfg="wfiMode",0. Set wifMode=0 to allow doze, when wifmode=1 do not enter doze. And wfiMode configure is saved in flash.
			slpManVoteSetDozeEnable api only write a variable in ram to disable doze temporary. Because it does not write flash, it is valid in only active/idle/doze/sleep1.
            When wakeup from sleep2/hibernate, doze is configured by the wfiMode flag. And user can call slpManVoteSetDozeEnable to reconfigure doze. 
			The variable behind slpManVoteSetDozeEnable is true by default, which means allow to enter doze.

* @param    en: true, allow enter doze
                false, do not enter doze state
*/
void slpManVoteSetDozeEnable(bool en);

/**
* @brief void slpManSetSlpTimeTrim(uint8_t nMs)
* @details  to wakeup 0-3 ms eariler in paging sleep. Normally do not need to call this api.
*/
void slpManSetSlpTimeTrim(uint8_t nMs);
...
#if defined TYPE_EC718M
/**
  \fn          slpManRet_t slpManPSRamVoteHybdDisable(slpManPsramHybdVote_e item);
  \brief       psram do not enter hybrid mode, need to be used in pair with slpManPsRamVoteHybdEnable
  \param[in]   item        define the vote source
  \return      error code.
*/
slpManRet_t slpManPSRamVoteHybdDisable(slpManPsramHybdVote_e item);
/**
  \fn          slpManRet_t slpManPSRamVoteHybdDisable(slpManPsramHybdVote_e item);
  \brief       psram allow to enter hybrid mode, need to be used in pair with slpManPsRamVoteHybdDisable
  \param[in]   item        define the vote source
  \return      error code.
*/
slpManRet_t slpManPSRamVoteHybdEnable(slpManPsramHybdVote_e item);
/**
  \fn          bool slpManIsPSRamHybdEnable(void);
  \brief       psram allow to enter hybrid mode or not
  \param[in]   item        define the vote source
  \return      true/false: psram allow/not allow to enter hybrid mode
*/
bool slpManIsPSRamHybdEnable(void);

/**
  \fn          uint32_t slpManCheckPSRamHybdBitmap(void);
  
  \brief       check psram hybrid mode vote bitmap
  \param[in]   
  \return      bitmap defined in slpManPsramHybdVote_e
*/
uint32_t slpManCheckPSRamHybdBitmap(void);
