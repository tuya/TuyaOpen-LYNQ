...
typedef struct PpposEntity
{
    ...
    uint32_t      needAct   :1; /* 0/1: need to activate pdp context or not? */
    uint32_t      isLanSusp :1; /* lan device is suspended or not? */
    uint32_t      rsvdBits  :30;
    struct netif  pppIf;
...
typedef struct AtosEntity
{
    ...
    DlPduBlockList_t outList;   /* for raw ppp via sw esc, fifo order */
}AtosEntity_t;
...
typedef struct SerialEntity
{
    ...
    UlPduBlockList_t inList;  /* for mux data if needed */
    DlPduBlockList_t outList; /* for mux data if needed */
