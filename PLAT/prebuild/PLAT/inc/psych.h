...
typedef struct {
	...
	Vpu_int16 *prevSamples;
	Vpu_int16 *prevSamplesS;
#else
	float *prevSamples;
	float *prevSamplesS;
#endif
...
typedef struct 
{
	...
	void(*PsyCalculate) (ChannelInfo *channelInfo, GlobalPsyInfo *gpsyInfo,
		...
		unsigned int numChannels);
#ifdef fixed_point_test
	void(*PsyBufferUpdate_fixedpoint) (FFT_Tables *fft_tables, GlobalPsyInfo * gpsyInfo, PsyInfo * psyInfo,
		short *newSamples_int, unsigned int bandwidth,
		int *cb_width_short, int num_cb_short);
#else
