...
typedef enum UlPduImsDataType_Tag
{
    UL_PDU_IMS_NONE              = 0,    /*the ul pdu without ims related data*/
    UL_PDU_IMS_CALL_SIGNALING    = 1,    /*the ul pdu with ims call signaling data*/
    UL_PDU_IMS_CALL_VOICE        = 2,    /*the ul pdu with ims call voice data (RTP data)*/
    UL_PDU_IMS_SMS               = 3,    /*the ul pdu with ims SMS signaling data*/
    UL_PDU_IMS_OTHER             = 4,    /*the ul pdu with other IMS UL PDU, such as register signaling, RTCP, and etc*/
    UL_PDU_IMS_DATA_TYPE_MAX     = 4    /* as used 3 bits, not max value is 4 */
}UlPduImsDataType;
...
typedef struct UlPduBlock_Tag
{
    ...
    UINT8       bSameMem   :1;      /* whether the UlPduBlock and "ptr" use the same memory,
                                    ...
                                    */
    UINT8       pppCrcNok  :1;      /* PPP deescape, whether CRC checked failed */
    ...
    */
    ...
    UINT32      imsDataType :3;     /*ims data type within the ul pdu*/ /*UlPduImsDataType*/
...
/* low layer link rsvd space for packet/frame encapsulation, e.g. cmux, spi hdr, and etc. */
#define LLLNK_DLPDU_RSVD_HDR_SIZE       16
#define LLLNK_DLPDU_RSVD_TAIL_SIZE      4
#define LLLNK_DLPDU_CONSUMED_SIZE(sz)  (LLLNK_DLPDU_RSVD_HDR_SIZE + (sz) + LLLNK_DLPDU_RSVD_TAIL_SIZE)
...
#define DL_PDU_BLOCK_INCR(dlpdu, n)  \
            do\
            {\
                ((DlPduBlock_t*)(dlpdu))->pPdu   += (n);\
                ((DlPduBlock_t*)(dlpdu))->length -= (n);\
            }while(0)

#define DL_PDU_BLOCK_DECR(dlpdu, n)  \
            do\
            {\
                EC_ASSERT(((uint8_t*)(dlpdu) + sizeof(DlPduBlock_t)) + (n) <= ((DlPduBlock_t*)(dlpdu))->pPdu, dlpdu, ((DlPduBlock_t*)(dlpdu))->pPdu, n);\
                ((DlPduBlock_t*)(dlpdu))->pPdu   -= (n);\
                ((DlPduBlock_t*)(dlpdu))->length += (n);\
            }while(0)
