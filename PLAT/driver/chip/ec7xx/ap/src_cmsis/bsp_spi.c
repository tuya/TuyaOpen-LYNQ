...
extern void SPI_DmaRxEvent(uint32_t event, SPI_RESOURCES *spi); //mbtk change
...
BSP_SPI_BSS_SECTION static DmaDescriptor_t __ALIGNED(16) SPI0_DMA_Descriptor;
...
#include "spiCodec.h"
BSP_SPI_BSS_SECTION static DmaDescriptor_t __ALIGNED(16) SPI1_DMA_Descriptor[TOTAL_CHAIN_COUNT];
#else
BSP_SPI_BSS_SECTION static DmaDescriptor_t __ALIGNED(16) SPI1_DMA_Descriptor;
#endif
...
#ifdef SPI_CODEC_ENABLE
        if (SPI_GetInstanceNumber(spi) == 1)
        {
            DMA_rigisterChannelCallback(spi->dma->tx_instance, spi->dma->tx_ch, spi->dma->rx_callback);
        }
#endif
    }
#endif
...
static void SPI_DefValueTxDmaConfig(SPI_RESOURCES *spi)
{
#if (RTE_SPI0_IO_MODE == DMA_MODE) || (RTE_SPI1_IO_MODE == DMA_MODE)
    ...
    dmaTxConfig.dataWidth = (DmaDataWidth_e)spi->info->data_width;
    dmaTxConfig.addressIncrement = DMA_ADDRESS_INCREMENT_NONE;
    dmaTxConfig.sourceAddress = (void *)&spi->info->xfer.def_val;
    dmaTxConfig.targetAddress = (void *)&(spi->reg->DR);
    dmaTxConfig.totalLength   = SPI_FIFO_DEPTH * spi->info->data_width;
    dmaTxConfig.burstSize = DMA_BURST_4_BYTES;

    extraConfig.stopDecriptorFetch       = false;
    extraConfig.enableStartInterrupt     = false;
    extraConfig.enableEndInterrupt       = false;
    extraConfig.nextDesriptorAddress     = (void *)(spi->dma->descriptor);

    DMA_buildDescriptor(spi->dma->descriptor, &dmaTxConfig, &extraConfig);

    DMA_resetChannel(spi->dma->tx_instance, spi->dma->tx_ch);
#endif
}

static void SPI_swReset(uint32_t instance)
{
    ...
    switch (state)
    {
...
#endif
            {
                CLOCK_setClockSrc(g_spiClocks[2*instance+1], ((instance == 0) ? FCLK_SPI0_SEL_26M : FCLK_SPI1_SEL_26M));
                CLOCK_clockDisable(CLK_HF612M);
        ...
        case ARM_POWER_FULL:
...
#endif
            {
                ...
                CLOCK_setClockSrc(g_spiClocks[2*instance+1], ((instance == 0) ? FCLK_SPI0_SEL_612M : FCLK_SPI1_SEL_612M));
            ...
            spi->reg->DRLR = EIGEN_VAL2FLD(SPI_DRLR_TX_REQ_LVL, 0) | EIGEN_VAL2FLD(SPI_DRLR_RX_REQ_LVL, 4);
            spi->reg->RPTCR = 0xFF;

            spi->reg->DRRCR = EIGEN_VAL2FLD(SPI_DRRCR_RX_WAIT_CYCLE, 0x1F) | \
                              EIGEN_VAL2FLD(SPI_DRRCR_READ_DEPTH_ONE_BURST, 0x3) | \
...
int32_t SPI_StopSend(SPI_RESOURCES *spi)
{
    SPI_DmaRxEvent(DMA_EVENT_END, spi);

    return ARM_DRIVER_OK; //mbtk change
}
...
#ifdef SPI_CODEC_ENABLE
        DMA_buildDescriptorChain(spi->dma->descriptor, &dmaTxConfig, TOTAL_CHAIN_COUNT, false, true, true);
        DMA_loadChannelDescriptorAndRun(spi->dma->tx_instance, spi->dma->tx_ch, spi->dma->descriptor);
        spi->dma->descriptor[TOTAL_CHAIN_COUNT/2-1].CMDR |= 1<<21;
...
int32_t SPI_Receive(void *data, uint32_t num, SPI_RESOURCES *spi)
{
    ...
    if(spi->dma)
    {
        ...
        else
        {
            ...
            DMA_loadChannelFirstDescriptor(spi->dma->tx_instance, spi->dma->tx_ch, (void *)spi->dma->descriptor);
            spi->reg->IMSC |= SPI_IMSC_RXIM_Msk; // trigger for sleep vote
        }
...
/**
  ...
  \return      \ref execution_status
*/
PLAT_FM_RAMCODE int32_t SPI_Transfer(const void *data_out, void *data_in, uint32_t num, SPI_RESOURCES *spi)
{
#ifdef PM_FEATURE_ENABLE
    uint32_t instance = SPI_GetInstanceNumber(spi);;
#endif
    ...
    {
        SPI_INFO *info = spi->info;
        SPI_TypeDef *spi_reg = spi->reg;

        spi_reg->CR1 |= SPI_CR1_SSE_Msk;
        ...
        while(info->xfer.num > info->xfer.rx_cnt)
        {
            /* tx flow, the differ between tx_cnt and rx_cnt shall be less than SPI_FIFO_DEPTH(16) to get rid of rx fifo overflow,
               for the max number of tx chunk is 16+1(fifo_depth + shift_register)
            */
            if((spi_reg->SR & SPI_SR_TNF_Msk) &&
               (info->xfer.tx_cnt < info->xfer.num) &&
               ((info->xfer.tx_cnt - info->xfer.rx_cnt) < SPI_FIFO_DEPTH))
            {
                if(data_width == 2U)
                    spi_reg->DR = *((uint16_t *)(info->xfer.tx_buf + (info->xfer.tx_cnt << 1U)));
                else
                    spi_reg->DR = info->xfer.tx_buf[info->xfer.tx_cnt];
                info->xfer.tx_cnt++;
            }

            // rx flow
            while(spi_reg->SR & SPI_SR_RNE_Msk)
            {
                if(data_width == 2U)
                    *((uint16_t *)(info->xfer.rx_buf + (info->xfer.rx_cnt << 1U))) = spi_reg->DR;
                else
                    info->xfer.rx_buf[info->xfer.rx_cnt] = (uint8_t)(spi_reg->DR);
    ...
    else if(spi->dma)
    {
        ...
        dmaTxConfig.targetAddress = (void *)&(spi->reg->DR);
        dmaTxConfig.totalLength   = num * data_width;
        dmaTxConfig.burstSize = (data_width == 2)? DMA_BURST_8_BYTES : DMA_BURST_4_BYTES;
        ...
        dmaRxConfig.totalLength   = num * data_width;
        dmaRxConfig.burstSize = (data_width == 2)? DMA_BURST_8_BYTES : DMA_BURST_4_BYTES;
...
int32_t SPI_Control(uint32_t control, uint32_t arg, SPI_RESOURCES *spi)
{
    ...
    switch(control & ARM_SPI_CONTROL_Msk)
    {
        ...
        case ARM_SPI_SET_DEFAULT_TX_VALUE:
            spi->info->xfer.def_val = (arg & 0xffff) | ((arg & 0xffff) << 16);
            return ARM_DRIVER_OK;
...
void SPI_IRQHandler(SPI_RESOURCES *spi)
{
    ...
    while((spi->reg->SR & SPI_SR_RNE_Msk) && (info->xfer.rx_cnt < info->xfer.num))
    {
        // Send only
...
void SPI_DmaRxEvent(uint32_t event, SPI_RESOURCES *spi)
{
    // extern void spiSignalEvent(uint32_t event);
    // spiSignalEvent(0xFF00 | event);
#if (RTE_SPI0_IO_MODE == DMA_MODE) || (RTE_SPI1_IO_MODE == DMA_MODE)
#ifdef PM_FEATURE_ENABLE
    uint32_t instance = SPI_GetInstanceNumber(spi);
#endif
    ...
    switch(event)
    {
        ...
        case DMA_EVENT_END:
            ...
            if(spi->info->cb_event)
            {
                if (SPI_GetInstanceNumber(spi) != 1)
                {
                    spi->info->cb_event((event == DMA_EVENT_END) ? ARM_SPI_EVENT_TRANSFER_COMPLETE : ARM_SPI_EVENT_RX_TIMEOUT);
                }
            }
#ifdef PM_FEATURE_ENABLE
            CHECK_TO_UNLOCK_SLEEP(instance);
...
static int32_t SPI0_StopSend(void)
{
    return SPI_StopSend(&SPI0_Resources);
}
...
static int32_t SPI1_StopSend(void)
{
    return SPI_StopSend(&SPI1_Resources);
}
