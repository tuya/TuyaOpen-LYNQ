...
#ifdef PLAT_PA_RAMCODE
#undef PLAT_PA_RAMCODE
#define PLAT_PA_RAMCODE
#endif
...
int32_t USART_Receive(void *data, uint32_t num, USART_RESOURCES *usart)
{
    ...
    if((usart->io_mode >> USART_RX_IO_MODE_POS) == DMA_MODE)
    {
        ...
        // If there's lingering data in rxfifo, re-trigger timeout count
        if(usart->reg->FSR >> USART_FSR_RXFIFO_WL_Pos)
        {
            usart->reg->TCR = USART_TCR_TOCNT_SWTRG_Msk;
        }
