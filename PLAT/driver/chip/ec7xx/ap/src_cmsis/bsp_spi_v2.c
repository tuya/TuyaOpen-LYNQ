...
static int32_t SPI_SetBusSpeed(uint32_t bps, SPI_RESOURCES *spi)
{
    ...
    if (bps >= 26000000)
    {
        spi->reg->MPSR |= (0x1UL << SPI_MPSR_MRX_PHA_SEL_Pos);
    }
#endif
...
int32_t SPI_Control(uint32_t control, uint32_t arg, SPI_RESOURCES *spi)
{
    ...
    switch(control & ARM_SPI_CONTROL_Msk)
    {
        ...
        case ARM_SPI_MODE_MASTER:
            ...
            if (SPI_GetInstanceNumber(spi) == 1)
            {
                spi->reg->TRCR = (spi->reg->TRCR & ~SPI_TRCR_MAS_RX_STOP_LEVEL_Msk) | EIGEN_VAL2FLD(SPI_TRCR_MAS_RX_STOP_LEVEL, 0xFF);
            }
            else
#endif
            {
