...
#define CAN_TX_ISSUED_FLAG       BIT(1)
...
static uint32_t CAN_GetClock(CAN_RESOURCES *can)
{
    ...
    return GPR_getClockFreq(CLK_APB_MP);
#endif
...
Ref configuration:
  ...
  bitrate(KHz)     brp      Tq(us)    prop_seg      phase_seg1     phase_seg2        NBT      Sample point
  1000(1003.92)     6        1/17        6              6              4             17           76.4%
   500(501.96)     12        2/17        6              6              4             17           76.4%
   250(250.98)     24        4/17        6              6              4             17           76.4%
     200           32        5/16        6              6              3             16           81.3%
   125(125.49)     48        8/17        6              6              4             17           76.4%
     100           64        5/8         6              6              3             16           81.3%
...
Ref configuration:
  ...
  1000(1003.92)     6        1/17        6              6              4             17           76.4%
   500(501.96)     12        2/17        6              6              4             17           76.4%
   250(250.98)     24        4/17        6              6              4             17           76.4%
   200(199.61)     54        5/19        8              6              4             19           78.9%
*/
...
static int32_t CAN_MessageSend (uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, const uint8_t *data, uint8_t size, CAN_RESOURCES *can)
{
    ...
    // Fix issue that tx isr is lost when tx and rx irq occur at the same time
    mask = SaveAndSetIRQMask();
    can->reg->CMDR = cmd;
    can->ctrl->pending |= CAN_TX_ISSUED_FLAG;
    RestoreIRQMask(mask);
...
void CAN_IRQHandler(CAN_RESOURCES *can)
{
    ...
    while((isr = reg->ISR) && (n < CAN_MAX_IRQ_ROUNDS))
    {
        ...
        if((ctrl->pending & CAN_TX_ISSUED_FLAG) && (str & CAN_STR_TX_COMPLETE_Msk))
        {
            isr |= CAN_ISR_TX_EMPTY_Msk;
            ctrl->pending &=~CAN_TX_ISSUED_FLAG;
        }
