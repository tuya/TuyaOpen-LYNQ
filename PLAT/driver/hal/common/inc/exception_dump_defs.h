...
typedef struct _EPAT_PLAT_stack_Psp
{
    UINT32 ecStack[80];
}PLAT_stack_Psp;

typedef struct _EPAT_PLAT_stack_Msp
{
    UINT32 ecStack[32];
}PLAT_stack_Msp;
 ...
 * HIB time enum ID
*/
typedef enum _EPAT_Osa_HibTimerIdEnum
{
    OSA_PS_HIB_PLMN_PERIOD_OR_OOS_TIMER = 1,
    OSA_PS_HIB_EMM_T3245_TIMER,
    OSA_PS_HIB_EMM_T3247_TIMER,
    OSA_PS_HIB_EMM_T3324_TIMER,
    OSA_PS_HIB_EMM_T3325_TIMER,
    OSA_PS_HIB_EMM_T3346_TIMER,
    OSA_PS_HIB_EMM_T3402_TIMER,
    OSA_PS_HIB_EMM_T3411_TIMER,
    OSA_PS_HIB_EMM_T3412_TIMER,
    OSA_PS_HIB_EMM_T3448_TIMER = 10,
    OSA_PS_HIB_EMM_FORBIDDEN_TA_TIMER_ID,
    OSA_PS_HIB_EMM_BLOCK_PLMN_TIMER_ID,
    //PS_HIB_ESM_T3396_TIMER,
    OSA_PS_HIB_POWER_ON_DELAY_TIMER,
    OSA_PS_HIB_EMM_EMERGENCY_CAMP_TIMER_ID,

    OSA_PS_HIB_ERRC_T320_TIMER = 30,
    OSA_PS_HIB_ERRC_T325_TIMER,
    OSA_PS_HIB_ERRC_T330_TIMER,
    OSA_PS_HIB_ERRC_BCCH_MOD_TIMER,
    OSA_PS_HIB_ERRC_LOG_INTERVAL_TIMER,
    OSA_PS_HIB_ERRC_JD_INTERVAL_TIMER,

    /*40 ~ 50 is pre defined for IMS used */
    OSA_PS_IMS_HIB_BASE_TIMER       = 40,
    OSA_PS_IMS_HIB_MAX_TIMER        = 50,

    OSA_PS_HIB_MAX_TIMER            = 0x40     /* 64 HIB timer resversed for PS */
...
typedef struct _EPAT_PS_EmmFlags
{
    ...
    Osa_HibTimerId              fplmnTimer;             /* While store the PLMN id in "forbidden PLMN list" or "forbidden PLMNs for GPRS service" list,
                                                           which timer is started(T3245 or T3247) */
}PS_EmmFlags;
...
//use CUST DUMP FLASH PART1
typedef struct _EPAT_PS_PART16_CerrcUeCapability
{
    UINT8                                       asRelease;
    UINT8                                       ueCategory;
    UINT8                                       restrictOnUseOfCE       : 1;
    UINT8                                       duplexMode              : 1;
    UINT8                                       bTestSim                : 1;
    UINT8                                       bUeCapaLimitToRel9      : 1;
    UINT8                                       bEnableSibRevOpt        : 1;
    UINT8                                       bUltraHighSpeed         : 1;
    UINT8                                       rsvd1                   : 2;
    BOOL                                        bRaiSupport;
    ...
    BOOL                                        bEnableSps;
    BOOL                                        bEnableTtiBundling;
    BOOL                                        bCfunClrBarCell;
...
}PS_PART16_CerrcUeCapability;

//use CUST DUMP FLASH PART2
typedef struct _EPAT_PS_PART17_CerrcDedicatedPriority
{
    //3 CerrcDeprioritisationFreqList
    struct
    {
        UINT32                                  t325StartTime;
        UINT8                                   t325Len;
        UINT8                                   rsvd[2];
        UINT8                                   numOfFreq;
        UINT32                                  depriFreqList[8];
    }depriFreqList;

    //3 CerrcIdleMobilityCtrlInfo
    struct
    {
        UINT32                                  t320StartTime;
        UINT8                                   t320Len;
        UINT8                                   rsvd[2];
        UINT8                                   numOfFreq;
        UINT32                                  freqList[16];
        UINT8                                   freqPriorityList[16];
    }idleMobilityCtrlInfo;
}PS_PART17_CerrcDedicatedPriority;

typedef struct _EPAT_PS_CERRC_InterFreqInfoTag
{
    UINT32                                      carrierFreqInter;
    UINT8                                       tReselectionInter;
    INT8                                        interFreqPriority;
    UINT8                                       threshXHigh;
    UINT8                                       threshXLow;
    UINT8                                       threshX_QPresent;
    UINT8                                       threshXHighQ;
    UINT8                                       threshXLowQ;
    UINT8                                       rsvd;
}PS_CERRC_InterFreqInfo;

typedef struct _EPAT_PS_CERRC_SibxInfoTag
{
    //4 SIB3
    INT8                                        intraFreqPriority;
    UINT8                                       threshServingLow;
    UINT8                                       threshServingLowQ;
    UINT8                                       threshServingLowQPresent : 1;

    //4 SIB5
    UINT8                                       numOfInterFreq : 7;
    PS_CERRC_InterFreqInfo                      interFreqInfoList[8];
}PS_CERRC_SibxInfo;

//CerrcCellBarredCause
typedef UINT8 PS_CERRC_CellBarredCause_t;
typedef enum _EPAT_PS_CERRC_CellBarredCauseTag
{
    PS_CERRC_CELL_NOT_BARRED = 0,
    PS_CERRC_CELL_BARRED_RA_FAIL = 1,
    PS_CERRC_CELL_BARRED_T300_EXPIRED = 2,
    PS_CERRC_CELL_BARRED_BCH_CRC_ERROR = 3,
    PS_CERRC_CELL_BARRED_ACCESS_BARRED = 4,
    PS_CERRC_CELL_BARRED_AUTH_FAIL = 5,
    PS_CERRC_CELL_BARRED_PHY_ESCAPE = 6,
    PS_CERRC_CELL_BARRED_ACCESS_REJECTED = 7,
    PS_CERRC_CELL_BARRED_RADIO_LINK_FAILURE = 8,
    PS_CERRC_CELL_BARRED_BY_NAS_IND = 9,
    PS_CERRC_CELL_BARRED_BY_ATCMD = 10,
    PS_CERRC_CELL_BARRED_PHY_IND_WEAK_NCELL = 11,
    PS_CERRC_CELL_BARRED_PHY_RESET = 12,
    PS_CERRC_CELL_BARRED_NCELL_BCH_CRC_ERROR = 13,
    PS_CERRC_CELL_BARRED_IDLE_UNRELIABLE = 14,
}PS_CERRC_CellBarredCause;

typedef struct _EPAT_PS_CERRC_BarCellInfoTag
{
    UINT32                                      carrierFreq;
    UINT16                                      phyCellId;
	PS_CERRC_CellBarredCause_t                  barCause;
}PS_CERRC_BarCellInfo;

typedef struct _EPAT_PS_CERRC_CellDBInfoTag
{
    UINT32                                      carrierFreq : 24;
    UINT32                                      barredByPlmn: 1;
    UINT32                                      barredByTa  : 1;
    UINT32                                      rankValid   : 1;
    UINT32                                      rsvd        : 5;
...
//use CUST DUMP FLASH PART3
typedef struct _EPAT_PS_PART18_CerrcCellDumpInfo
{
    PS_CERRC_SibxInfo                           sibxInfo;
    PS_CERRC_BarCellInfo                        barCellInfo[12];
    PS_CERRC_CellDBInfo                         cellDBInfo[8];
}PS_PART18_CerrcCellDumpInfo;
...
typedef struct _EPAT_PhyUeCapabilityTag
{
    ...
    UINT8                               qualityLevel;
    //High Tx power report threshold, [-45...23]dbm; if highTxTheshold is 127, means it's not configured
    INT8                                highTxTheshold;
    //Low Tx power report threshold, [-45...23]dbm; if lowTxTheshold is 127, means it's not configured
    INT8                                lowTxTheshold;
    // tx power report function enable. set to false if highTxTheshold or lowTxTheshold set to 127
    UINT8                               txPwrRptEnable;
