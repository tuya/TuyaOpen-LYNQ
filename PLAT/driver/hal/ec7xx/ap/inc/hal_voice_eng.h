...
/*
 *ap master owner flag, used to mark who is currently using cp amr
 *cp return it back to ap transparently
*/
typedef enum
{
    HAL_AMR_OWNER_IMS   = 0,
...
typedef struct
{
	...
	UINT8	    *pTonePcmData;  /* output tone buf address,RO for CP*/
    UINT8       *pPcmData;      /* output PCM data address */
    UINT8       *pAmrData;      /* return to AP, AP need to free it */
    UINT8       *pPreVemPcmData;/* input PCM buffer for PCM+VEM play path, AP need to free it*/
    void        *pExtra0;
    void        *pExtra1;
...
void halamrRecordIpc(uint8_t codecType, uint8_t* recordBuf, uint8_t* amrBuf, uint8_t* ulpduAddr, uint8_t* postVemPcmData, uint8_t* refVemPcmData);
