...
// Set the detection time for cellular module without data transmission. If the set time is exceeded, the cellular module will actively release RRC. The purpose is to further optimize power consumption. The smaller the time, the more likely it is to cause data packet loss, use with caution.
	...
	/* Handled by tkl_cellular_base_ioctl() in this generation of the adapter;
	 * appended so the existing command values keep their numbering. */
	CELL_IOCTL_GET_SINGAL_QUALITY,
	CELL_IOCTL_GET_CURRENT_OPERATOR,
	CELL_IOCTL_SET_LOGSEL,
}CELL_IOCTRL_CMD_E;
 ...
 */
typedef void (*TKL_REGISTION_NOTIFY)(uint8_t sim_id,
                                     TUYA_CELLULAR_MDS_STATUS_E st);

/* Signal quality as reported by tkl_cellular_base_get_singal_quality():
...
/**
 * @brief Get the signal quality of the SIM's channel
 * @param sim_id SIM card ID
 * @param quality signal quality
 * @return 0 success, others failure
 */
OPERATE_RET tkl_cellular_base_get_singal_quality(
    uint8_t sim_id, cellular_signal_quality_t *quality);
